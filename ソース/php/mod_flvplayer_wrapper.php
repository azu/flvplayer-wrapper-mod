<?php
//ソースコード改変前にフォーマッターにかけたので
//インデントはあらかた直っていると思いますが
//逆に見にくくなってる箇所も一部あります
...
case 0:
    main_bar = main_bar0;
    main_bar1._visible = false;
    design_mode0_so.data.flag = true;
    break;
case 1:
    main_bar = main_bar1;
    main_bar0._visible = false;
...
//raw_user_id  = 今のところundefinedしか送信されていない？ //vpos      = chatノードのvpos (_vposは秒　vpos = _vpos * 100)
//date      = chatノードのdate
...
// n g_idsは新着メッセージの検索処理を最優先させるために、常にuser_idの小さい順にソートしておく
//↑ng_ids[n].フィールド名
//user_id ID
...
// 通常時のタブメニューの座標
var tabmenu_default_x;
var tabmenu_default_y;
var tabmenu_default_width;
var tabmenu_default_height;
// 初期プレイヤサイズ
var root_base_w;
var root_base_h;
// 動画の基準サイズ
var video_base_w;
var video_base_h;
var video_normal_width;
var video_normal_offset = 0;
var video_screen_mode = 0;
var video_aspect_ratio = 4 / 3;
var video_wide_cut = false;
...
var now_push_out_inputarea = false;
var click_time = getTimer(); // クリック時の時間
var click_x = _root._xmouse; // クリック時の位置
var click_y = _root._ymouse; // クリック時の位置
var click_id = false; // ビデオウインドウのIDクリック処理を行ったときtrue
var click_valid = false; // ビデオウインドウのクリックを有効にしていいときtrue
var double_click_valid = false; // ビデオウインドウのダブルクリックを有効にしていいときtrue
var key_down = false; // キーが押されているときtrue
var key_down_thumb = false; // サムネイル表示キーが押されているときtrue
var movie_loaded = false; // ムービーが開始可能までロードされたときtrue
var movie_started = false; // ムービーが開始済みのときtrue
var mouse_down = false; // マウスが押されているときtrue
...
var mouse_on_context_menu = false; // マウスがContextMenuにあるときtrue
var mouse_on_drag_tabmenu = false; // マウスがドラッグ可能なnico.tabmenuの上にあるときtrue
var mouse_drag_resize_x = false; // マウスがplayerを横リサイズ中のときtrue
var mouse_drag_resize_y = false; // マウスがplayerを縦リサイズ中のときtrue
var mouse_drag_tabmenu = false; // マウスがnico.tabmenuをドラッグ中のときtrue
// コンテキストメニュー定義
if (useswfversion >= 7) {
    ...
    var screen_full = new ContextMenuItem("全画面表示");
    ...
    var screen_max = new ContextMenuItem("最大化表示");
    screen_max.enabled = false;
    //screen_max.visible = false;
    screen_max.separatorBefore = false;
    screen_max.onSelect = function() {
    ...
    var screen_normal = new ContextMenuItem("サイズを戻す");
    screen_normal.enabled = false;
    //screen_normal.visible = false;
    screen_normal.separatorBefore = false;
    ...
    var cmi_aspect_16_9_cut = new ContextMenuItem("比率変更 16:9 (上下カット)");
    cmi_aspect_16_9_cut.enabled = false;
    //cmi_aspect_16_9_cut.visible = false;
    cmi_aspect_16_9_cut.separatorBefore = false;
    cmi_aspect_16_9_cut.onSelect = function() {
        changeAspect(16, 9, true);
    };
    var cmi_switch_add_id_overlay = new ContextMenuItem("コメントID表示");
    cmi_switch_add_id_overlay.enabled = false;
    ...
    var cmi_switch_no_add_id_overlay = new ContextMenuItem("コメントID非表示");
    cmi_switch_no_add_id_overlay.enabled = false;
    cmi_switch_no_add_id_overlay.visible = false;
    cmi_switch_no_add_id_overlay.separatorBefore = true;
    cmi_switch_no_add_id_overlay.onSelect = function(obj, menu) {
        if (add_id_overlay) {
...
var first_time_fullmonitor = false;
...
var transparent_commentlist = false;
...
var timed_hide_commentlist = false;
var timed_hide_timelimit = 2;
var click_pause = false;
var double_click_fullmonitor = false;
var double_click_fullscreen = false;
var drag_resize = false;
...
var check_autojump = false;
var load_NiconiCM = false;
var autopop_cm = false;
var spread_wide_player = true;
...
var double_click_fullmonitor_so = SharedObject.getLocal("double_click_fullmonitor");
if (double_click_fullmonitor_so.data.flag != undefined) {
    double_click_fullmonitor = Boolean(double_click_fullmonitor_so.data.flag);
}
...
var drag_resize_so = SharedObject.getLocal("drag_resize");
if (drag_resize_so.data.flag != undefined) {
    drag_resize = Boolean(drag_resize_so.data.flag);
}
...
if (first_time_full_so.data.flag != undefined) {
    ...
    first_time_full_so.data.flag = first_time_full;
}
var first_time_fullmonitor_so = SharedObject.getLocal("first_time_fullmonitor");
if (first_time_fullmonitor_so.data.flag != undefined) {
    first_time_fullmonitor = Boolean(first_time_fullmonitor_so.data.flag);
} else {
    first_time_fullmonitor_so.data.flag = first_time_fullmonitor;
}
...
var transparent_commentlist_so = SharedObject.getLocal("transparent_commentlist");
if (transparent_commentlist_so.data.flag != undefined) {
    transparent_commentlist = Boolean(transparent_commentlist_so.data.flag);
}
//vartimed_hide_commentlist_so = SharedObject.getLocal("timed_hide_commentlist");
//if(timed_hide_commentlist_so.data.flag != undefined){timed_hide_commentlist = Boolean(timed_hide_commentlist_so.data.flag);}
timed_hide_commentlist = transparent_commentlist;
...
if (auto_smoothing_so.data.flag != undefined) {
    ...
    auto_smoothing_so.data.flag = auto_smoothing;
}
...
if (auto_smoothing_off_so.data.flag != undefined) {
    ...
    auto_smoothing_off_so.data.flag = auto_smoothing_off;
}
...
var check_autojump_so = SharedObject.getLocal('check_autojump');
if (check_autojump_so.data.flag != undefined) {
    check_autojump = Boolean(check_autojump_so.data.flag);
}
...
var spread_wide_player_so = SharedObject.getLocal('spread_wide_player');
if (spread_wide_player_so.data.flag != undefined) {
    spread_wide_player = Boolean(spread_wide_player_so.data.flag);
}
// テスト
...
if (useswfversion >= 7) {
    ...
    nico_listener.onLoadInit = function(video) {
        ...
        // サムネイルをロード
        //if( VIDEO.substr( 0, 2 ) == "sm" || VIDEO.substr( 0, 2 ) == "nm" )
        {
            thumb_view = nico.videowindow.createEmptyMovieClip("thumb", getPrevDepth(nico.videowindow.titleContainer));
            thumb_view._visible = false;
            var listenerObject = new Object();
            listenerObject.onLoadInit = function() {
                showThumb();
            };
            var bg_mcl = new MovieClipLoader();
            bg_mcl.loadClip(thumbImage, thumb_view);
            bg_mcl.addListener(listenerObject);
        }
    };
    ...
    //if( VIDEO.substr( 0, 2 ) == "sm" || VIDEO.substr( 0, 2 ) == "nm" )
    {
        setTimeout(thumbLoadTimer, 500);
    }
}
// flvplayerの位置調整
//var fwOffsetY = 30;
//nico._y = -fwOffsetY;
/* //左上のx,yを指定するバージョン function createSquareBtn(path,obj_name,label,text_fmt,depth,x,y,w,h,color){ //path 場所 obj_name 名前 labelボタンにつける文字 text_fmtその文字のtextFormat //w,h ボタンのwidth height //depth ボタンの深度 //x,y ボタン左上のx,y座標 //color 色 16進数 0xffffff //path=path obj_name="name"にすると //path.btnを指定色で四角に描画して、path.btn.lableをボタン中央に配置する var btn = path.createEmptyMovieClip(obj_name, depth); createSquare(btn,x,y,w,h,color); //	if(label != ""){ btn.createTextField("name",1,0,0,1,1); btn.name.type = "dynamic"; btn.name.border = false; btn.name.selectable = false; btn.name.background = false; btn.name.autoSize = true; btn.tabEnabled = false; btn.name.text = label; btn.name.setTextFormat(text_fmt); //	} btn.onRollOver = function(){ this._alpha = 60; }; btn.onRollOut = function(){ this._alpha = 100; }; btn.onReleaseOutside = btn.onRollOut; } function createSquare(path,x,y,w,h,color){ //角がちょっと丸い四角をpathに描画する //x,y四角の左上の座標 w,h幅高さ color色 path._x = x; path._y = y; w = Math.floor(w/2) * 2; h = Math.floor(h/2) * 2; path.beginFill(color,100); path.moveTo(4,h); path.lineTo(w-4,h); path.curveTo(w,h,w,h-4); path.lineTo(w,4); path.curveTo(w,0,w-4,0); path.lineTo(4,0); path.curveTo(0,0,0,4); path.lineTo(0,h-4); path.curveTo(0,h,4,h); path.endFill(); } */
function createSquareBtn(path, obj_name, label, text_fmt, depth, x, y, w, h, color, note) {
    ...
    btn.onRollOver = function() {
        if (this.name.note) {
            clearInterval(nico.lineWindowShowIntervalId);
            nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, this.name.note);
        }
        this._alpha = 60;
    };
...
var white11b_fmt = new TextFormat();
white11b_fmt.font = user_font;
white11b_fmt.size = 11;
white11b_fmt.bold = true;
white11b_fmt.color = 0xFFFFFF;
white11b_fmt.align = "center";
white11b_fmt.rightMargin = 1;
white11b_fmt.leftMargin = 1;
white11b_fmt.leading = -4;
...
var thumb_fmt = new TextFormat();
thumb_fmt.font = user_font;
thumb_fmt.size = 14;
thumb_fmt.bold = true;
thumb_fmt.color = 0xffffff;
thumb_fmt.align = "center";
thumb_fmt.rightMargin = 2;
thumb_fmt.leftMargin = 2;
var timeviewer_fmt = new TextFormat();
timeviewer_fmt.font = user_font;
timeviewer_fmt.size = 13;
timeviewer_fmt.bold = true;
timeviewer_fmt.color = 0x000000;
timeviewer_fmt.align = "center";
timeviewer_fmt.rightMargin = 0;
timeviewer_fmt.leftMargin = 0;
// ★★★★★★★★★★★上のバー 2★★★★★★★★★★★
...
main_bar.base.onRelease = function() {
    ...
    if (quick_ngid_mode && nico.tabmenu.loglist_menu.add_id._visible) {
        ...
        if (xm > main_bar.main_info._x && xm < main_bar.main_info._x + main_bar.main_info._width) {
            ...
            if (ym > main_bar.main_info._y && ym < main_bar.main_info._y + main_bar.main_info._height) {
                nico.tabmenu.loglist_menu.add_id._visible = false;
...
main_bar.filter1.onRollOver = function() {
    ...
    clearInterval(nico.lineWindowShowIntervalId);
    nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, '上表示コメントフィルタ');
};
...
main_bar.filter2.onRollOver = function() {
    ...
    clearInterval(nico.lineWindowShowIntervalId);
    nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, '下表示コメントフィルタ');
};
...
main_bar.filter3.onRollOver = function() {
    ...
    clearInterval(nico.lineWindowShowIntervalId);
    nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, '大文字コメントフィルタ');
};
...
main_bar.filter4.onRollOver = function() {
    ...
    clearInterval(nico.lineWindowShowIntervalId);
    nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, '色文字コメントフィルタ');
};
...
main_bar.filter5.onRollOver = function() {
    ...
    clearInterval(nico.lineWindowShowIntervalId);
    nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, 'NGIDコメントフィルタ');
};
...
main_bar.filter_off.onRollOver = function() {
    clearInterval(nico.lineWindowShowIntervalId);
    nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, 'コメントフィルタOFF');
};
main_bar.filter_off.onRollOut = function() {
    clearInterval(nico.lineWindowShowIntervalId);
    nico.hideLineWindow();
};
// ダウンロード 35
createSquareBtn(main_bar, "download", "DL", white12b_fmt, 35, design.btn_dl.x[design_mode], 17, 30, 24, 0x303030, '動画のダウンロード');
...
// 削除メニュー 36
createSquareBtn(main_bar, "delete", "Del", white12b_fmt, 36, design.btn_del.x[design_mode], 17, 30, 24, 0x303030, 'キャッシュの削除');
main_bar.delete.onRelease = function() {
    if (local_server && movie_loaded) {
        var javascript_command = "new Ajax.Request('" + local_server_name[local_server_num] + "ajax_rm?" + videoId + "',{method:'get'});";
        // 読み込み完了前なら止めてしまう
        if (nico.player.stream_ns.bytesLoaded < nico.player.stream_ns.bytesTotal) {
            // 最大化解除
            if (nico.isLargeScreen || Stage["displayState"] == "fullScreen") {
                nico.controller.controller_submenu.NormalScreenButton.onRelease();
            }
            movie_loaded = false;
            ...
            nico.player.pause = undefined;
            nico.player.stream_ns.close();
            nico.player.onLoadError();
            for (var i = 0; i < 10; i++) {
                setTimeout(doJavaScript, 500 * i, javascript_command);
            }
        } else {
...
// 設定メニュー 37
createSquareBtn(main_bar, "pref_menu", "Opt", white12b_fmt, 37, design.btn_conf.x[design_mode], 17, 30, 24, 0x303030, '設定メニュー');
...
// NGID詳細 33
createSquareBtn(main_bar, "ngid_view", "NG", white12b_fmt, 33, design.btn_ngid.x[design_mode], 17, 30, 24, 0x303030, 'NG設定');
...
{
	// kakikomi詳細 34
	createSquareBtn( main_bar, "kakikomi_view", "ｶｷｺﾐ", white12b_fmt, 34, design.btn_kaki.x[design_mode], 17, 30, 24, 0x303030 );
...
// リンクボタン 40
createSquareBtn(main_bar, "link", "Lnk", white12b_fmt, 40, design.btn_link.x[design_mode], 17, 30, 24, 0x303030, '自動リンクの表示');
...
header.clock.onPress = function() {
    if (Key.isDown(1) || Stage["displayState"] == "fullScreen") {
...
function searchID() {
    ...
    for (var i = 0, l = Slot.length; i < l; i++) {
        // どのMessageSlotsをクリックしたのか調べるiループ
    ...
    if (id == undefined || id < 0) {
        ...
        for (var i = 0, l = raw_msgs.length; i < l; i++) {
            if (raw_msgs[i].user_id != id) {
                ...
                if (msgs[i]._slot != undefined) {
                    // 表示中のコメントならSlotsにも枠つける
...
ngid_menu.ok.onRelease = function() {
    ...
    for (var i = 0; i < mes_ary.length; i++) {
        // 27文字ピッタリ、もしくは数字だけのヤツを読み込む
...
function updateLogList(mode) {
    ...
    if (mode == "clear") {
        // 空欄にする
        nico.tabmenu.loglist_menu.cand_ng_id_list._visible = false;
        nico.tabmenu.loglist_menu.add_id._visible = false;
        if (list_mode != "normal") {
            if (always_back_to_normal_mode) {
                ...
                ClearLog();
                cand_ng_id = new Array();
                nico.tabmenu.loglist_menu.normal_list._visible = true;
            }
    ...
    if (mode == "cand_ng_id") {
        ...
        nico.tabmenu.loglist_menu.normal_list._visible = true;
        if (!e && !owner_thread_edit_mode) {
            nico.tabmenu.loglist_menu.add_id._visible = true;
        }
    ...
    if (mode == "normal") {
        ...
        cand_ng_id = new Array();
        nico.tabmenu.loglist_menu.cand_ng_id_list._visible = false;
        nico.tabmenu.loglist_menu.normal_list._visible = false;
        nico.tabmenu.loglist_menu.add_id._visible = false;
        ...
        for (var i = 0, j = 0, l = fwMessages.length; i < l; i++) {
            ...
            if (add_id) {
                // add_idならAddChatLogする前にメッセージにID付与
...
createToggleBtn(pref_menu, "show_info", "スクリーンに情報表示", black12_fmt, 15, 200, 67, 0x808080);
...
createToggleBtn(pref_menu, "add_id", "ログリストにID表示", black12_fmt, 19, 200, 92, 0x808080);
...
createToggleBtn(pref_menu, "clip_height", "ログリストを縮める", black12_fmt, 23, 200, 117, 0x808080);
...
createToggleBtn(pref_menu, "forbid_relation", "再生後オススメタブを出さない", black12_fmt, 26, 17, 142, 0x808080);
createToggleBtn(pref_menu, "change_title", "<title>タグをいぢる", black12_fmt, 27, 200, 142, 0x808080);
createToggleBtn(pref_menu, "hide_header", "ヘッダ部を消去", black12_fmt, 30, 17, 197, 0x808080);
pref_menu.hide_header.on.onRelease = function() {
    this._visible = false;
    hide_header = false;
    hide_header_so.data.flag = false;
    hide_header_so.flush();
    if (nico.isLargeScreen) {
        nico.stageListener.onResize();
    }
};
...
createToggleBtn(pref_menu, "push_out_inputarea", "入力部を押し出す", black12_fmt, 31, 135, 197, 0x808080);
pref_menu.push_out_inputarea.on.onRelease = function() {
    this._visible = false;
    push_out_inputarea = false;
    push_out_inputarea_so.data.flag = false;
...
pref_menu.transparent_header.on.onRelease = function() {
    this._visible = false;
    if (!transparent_inputarea && !transparent_commentlist) {
...
createToggleBtn(pref_menu, "transparent_inputarea", "入力部を透明化", black12_fmt, 33, 135, 222, 0x808080);
pref_menu.transparent_inputarea.on.onRelease = function() {
    this._visible = false;
    if (!transparent_header && !transparent_commentlist) {
        pref_menu.label_inputarea_alpha._visible = false;
        pref_menu.input_inputarea_alpha._visible = false;
        pref_menu.label_timed_hide_timelimit._visible = false;
        pref_menu.input_timed_hide_timelimit._visible = false;
...
createToggleBtn(pref_menu, "transparent_commentlist", "リストを透明化", black12_fmt, 34, 253, 222, 0x808080);
pref_menu.transparent_commentlist.on.onRelease = function() {
    this._visible = false;
    if (!transparent_header && !transparent_inputarea) {
        pref_menu.label_inputarea_alpha._visible = false;
        pref_menu.input_inputarea_alpha._visible = false;
        pref_menu.label_timed_hide_timelimit._visible = false;
        pref_menu.input_timed_hide_timelimit._visible = false;
    ...
    transparent_commentlist = false;
    transparent_commentlist.data.flag = false;
    transparent_commentlist.flush();
    timed_hide_commentlist = false;
    if (nico.isLargeScreen) {
        nico.stageListener.onResize();
    }
};
pref_menu.transparent_commentlist.off.onRelease = function() {
    this._parent.on._visible = true;
    pref_menu.label_inputarea_alpha._visible = true;
    pref_menu.input_inputarea_alpha._visible = true;
    pref_menu.label_timed_hide_timelimit._visible = true;
    pref_menu.input_timed_hide_timelimit._visible = true;
    transparent_commentlist = true;
    transparent_commentlist_so.data.flag = true;
    transparent_commentlist_so.flush();
    timed_hide_commentlist = true;
    if (nico.isLargeScreen) {
        nico.stageListener.onResize();
    }
};
createToggleBtn(pref_menu, "change_bgcolor", "背景を黒くする", black12_fmt, 35, 17, 252, 0x808080);
pref_menu.change_bgcolor.on.onRelease = function() {
    this._visible = false;
    if (nico.isLargeScreen) {
        bg._width = Math.ceil(Math.max(nico.videowindow_width, (video_base_w * nico.video_base_video._xscale / 100) + (nico.videowindow_width - nico.player.video_max_width)));
        bg._height = nico.videowindow_height;
    } else {
        bg._width = video_normal_width;
        bg._height = nico.videowindow_height;
    }
    bg._x = Math.floor((nico.videowindow_width - bg._width) / 2);
    bg._y = Math.floor((nico.videowindow_height - bg._height) / 2);
    change_bgcolor = false;
    change_bgcolor_so.data.flag = false;
    change_bgcolor_so.flush();
};
pref_menu.change_bgcolor.off.onRelease = function() {
    this._parent.on._visible = true;
    if (nico.isLargeScreen) {
        bg._width = Math.ceil(Math.max(nico.videowindow_width, (video_base_w * nico.video_base_video._xscale / 100) + (nico.videowindow_width - nico.player.video_max_width)));
        bg._height = nico.videowindow_height;
        bg._width = Math.max(bg._width, nico.player.getStageWidth());
        bg._height = Math.min((nico.inputArea._y + nico.inputArea._height) * 100 / nico.player._mc._yscale, Math.max(bg._height, nico.player.getStageHeight()));
        bg._y = -nico.player.VIDEOWINDOW_LARGE_DEFAULT_Y * 100 / nico.player._mc._yscale;
    } else {
        bg._width = video_normal_width;
        bg._height = nico.videowindow_height;
        bg._y = Math.floor((nico.videowindow_height - bg._height) / 2);
    ...
    bg._x = Math.floor((nico.videowindow_width - bg._width) / 2);
    change_bgcolor = true;
    change_bgcolor_so.data.flag = true;
    change_bgcolor_so.flush();
};
createToggleBtn(pref_menu, "first_time_full", "再生開始最大化", black12_fmt, 36, 17, 280, 0x808080);
pref_menu.first_time_full.on.onRelease = function() {
    pref_menu.first_time_full.on._visible = false;
    first_time_full = false;
    first_time_full_so.data.flag = false;
    first_time_full_so.flush();
    pref_menu.first_time_fullmonitor.on._visible = false;
    first_time_fullmonitor = false;
    first_time_fullmonitor_so.data.flag = false;
    first_time_fullmonitor_so.flush();
};
pref_menu.first_time_full.off.onRelease = function() {
    pref_menu.first_time_full.on._visible = true;
    first_time_full = true;
    first_time_full_so.data.flag = true;
    first_time_full_so.flush();
    if (first_time_fullmonitor_so.data.flag) {
        pref_menu.first_time_fullmonitor.on._visible = false;
        first_time_fullmonitor = false;
        first_time_fullmonitor_so.data.flag = false;
        first_time_fullmonitor_so.flush();
...
createToggleBtn(pref_menu, "first_time_fullmonitor", "再生開始全画面", black12_fmt, 37, 135, 280, 0x808080);
pref_menu.first_time_fullmonitor.on.onRelease = pref_menu.first_time_full.on.onRelease;
pref_menu.first_time_fullmonitor.off.onRelease = function() {
    pref_menu.first_time_full.on.onRelease();
    pref_menu.first_time_fullmonitor.on._visible = true;
    first_time_fullmonitor = true;
    first_time_fullmonitor_so.data.flag = true;
    first_time_fullmonitor_so.flush();
};
createToggleBtn(pref_menu, "end_time_normal", "再生終了最小化", black12_fmt, 38, 253, 280, 0x808080);
...
createToggleBtn(pref_menu, "spread_wide_player", "ワイドプレイヤを許可", black12_fmt, 41, 200, 312, 0x808080);
pref_menu.spread_wide_player.on.onRelease = function() {
    pref_menu.spread_wide_player.on._visible = false;
    spread_wide_player = false;
    spread_wide_player_so.data.flag = false;
    spread_wide_player_so.flush();
    if (!nico.isLargeScreen) {
        changeScale(video_screen_mode);
    }
};
pref_menu.spread_wide_player.off.onRelease = function() {
    pref_menu.spread_wide_player.on._visible = true;
    spread_wide_player = true;
    spread_wide_player_so.data.flag = true;
    spread_wide_player_so.flush();
    if (!nico.isLargeScreen) {
        changeScale(video_screen_mode);
    }
...
createToggleBtn(pref_menu, "auto_smoothing_off", "自動スムージング", black12_fmt, 53, 17, 67, 0x808080);
pref_menu.auto_smoothing_off.on.onRelease = function() {
    this._visible = false;
    if (auto_smoothing_so.data.flag) {
...
createToggleBtn(pref_menu, "auto_smoothing", "低画質のみ自動スムージング", black12_fmt, 54, 160, 67, 0x808080);
...
pref_menu.auto_smoothing.off.onRelease = function() {
    this._parent.on._visible = true;
    if (auto_smoothing_off_so.data.flag) {
        pref_menu.auto_smoothing_off.off.onRelease();
    }
    auto_smoothing = true;
    auto_smoothing_so.data.flag = true;
    auto_smoothing_so.flush();
};
createToggleBtn(pref_menu, "smoothing", "スムージングON （この動画に対して）", black12_fmt, 55, 17, 92, 0x808080);
...
createToggleBtn(pref_menu, 'load_NiconiCM', 'ニコニ広告の提供を表示', black12_fmt, 60, 17, 262, 0x808080);
...
createToggleBtn(pref_menu, 'hide_icon_cm', '＠CMアイコンを隠す', black12_fmt, 66, 17, 287, 0x808080);
...
createToggleBtn(pref_menu, 'check_autojump', '自動ジャンプの代わりにリンク表示', black12_fmt, 65, 17, 312, 0x808080);
/*コメントタブ 70*/
...
createSquareBtn(pref_menu, "apply_filter", "→ 今すぐ適用", white12b_fmt, 84, 270, 225, 100, 18, 0x303030);
...
createToggleBtn(pref_menu, "copy_title", "DL時タイトルをコピー", black12_fmt, 102, 190, 42, 0x808080);
createToggleBtn(pref_menu, "download_blank", "別窓でDLする(エラー対策)", black12_fmt, 103, 190, 67, 0x808080);
...
createToggleBtn(pref_menu, "double_click_fullscreen", "ダブルクリックで最大表示", black12_fmt, 113, 17, 247, 0x808080);
pref_menu.double_click_fullscreen.on.onRelease = function() {
    pref_menu.double_click_fullscreen.on._visible = false;
    double_click_fullscreen = false;
    double_click_fullscreen_so.data.flag = false;
    double_click_fullscreen_so.flush();
    pref_menu.double_click_fullmonitor.on._visible = false;
    double_click_fullmonitor = false;
    double_click_fullmonitor_so.data.flag = false;
    double_click_fullmonitor_so.flush();
};
pref_menu.double_click_fullscreen.off.onRelease = function() {
    pref_menu.double_click_fullscreen.on._visible = true;
    double_click_fullscreen = true;
    double_click_fullscreen_so.data.flag = true;
    double_click_fullscreen_so.flush();
    if (double_click_fullmonitor_so.data.flag) {
        pref_menu.double_click_fullmonitor.on._visible = false;
        double_click_fullmonitor = false;
        double_click_fullmonitor_so.data.flag = false;
        double_click_fullmonitor_so.flush();
...
createToggleBtn(pref_menu, "double_click_fullmonitor", "ダブルクリックで全画面表示", black12_fmt, 114, 190, 247, 0x808080);
pref_menu.double_click_fullmonitor.on.onRelease = pref_menu.double_click_fullscreen.on.onRelease;
pref_menu.double_click_fullmonitor.off.onRelease = function() {
    pref_menu.double_click_fullscreen.on.onRelease();
    pref_menu.double_click_fullmonitor.on._visible = true;
    double_click_fullmonitor = true;
    double_click_fullmonitor_so.data.flag = true;
    double_click_fullmonitor_so.flush();
};
createToggleBtn(pref_menu, "drag_resize", "プレイヤの外側あるいはShift押しドラッグでサイズ変更", black12_fmt, 115, 17, 272, 0x808080);
createToggleBtn(pref_menu, "click_ime_off", "IMEを画面クリックでOFF", black12_fmt, 116, 17, 297, 0x808080);
/*デバッグタブ*/
...
pref_menu.label_pref_on_fullscreen.text = "最大化時の設定";
...
pref_menu.createTextField("label_inputarea_alpha", 370, 258, 167, 1, 1);
pref_menu.createTextField("input_inputarea_alpha", 380, 315, 167, 30, 17);
pref_menu.createTextField("label_timed_hide_timelimit", 390, 258, 189, 1, 1);
pref_menu.createTextField("input_timed_hide_timelimit", 400, 315, 189, 30, 17);
...
function updatePrefMenu(num) {
    ...
    if (num == 0) {
        ...
        pref_menu.transparent_commentlist._visible = true;
        if (transparent_commentlist) {
            pref_menu.transparent_commentlist.on._visible = true;
        } else {
            pref_menu.transparent_commentlist.on._visible = false;
        }
        ...
        pref_menu.first_time_fullmonitor._visible = true;
        if (first_time_fullmonitor_so.data.flag) {
            pref_menu.first_time_fullmonitor.on._visible = true;
        } else {
            pref_menu.first_time_fullmonitor.on._visible = false;
        ...
        if (first_time_full_so.data.flag && !first_time_fullmonitor_so.data.flag) {
            pref_menu.first_time_full.on._visible = true;
        } else {
            pref_menu.first_time_full.on._visible = false;
        }
        ...
        pref_menu.spread_wide_player._visible = true;
        //if( spread_wide_player_so.data.flag )
        if (spread_wide_player) {
            pref_menu.spread_wide_player.on._visible = true;
        } else {
            pref_menu.spread_wide_player.on._visible = false;
        }
    ...
    if (num == 1) {
        ...
        pref_menu.check_autojump._visible = true;
        if (check_autojump) {
            pref_menu.check_autojump.on._visible = true;
        } else {
            pref_menu.check_autojump.on._visible = false;
        }
    ...
    if (num == 3) {
        ...
        pref_menu.double_click_fullmonitor._visible = true;
        if (double_click_fullmonitor) {
            pref_menu.double_click_fullmonitor.on._visible = true;
        } else {
            pref_menu.double_click_fullmonitor.on._visible = false;
        }
        pref_menu.double_click_fullscreen._visible = true;
        if (double_click_fullscreen && !double_click_fullmonitor) {
            pref_menu.double_click_fullscreen.on._visible = true;
        } else {
            pref_menu.double_click_fullscreen.on._visible = false;
        }
        pref_menu.drag_resize._visible = true;
        if (drag_resize) {
            pref_menu.drag_resize.on._visible = true;
        } else {
            pref_menu.drag_resize.on._visible = false;
        }
...
function updateLinkThumb(mode, l_num, t_num) {
    if (mode == "add") {
        ...
        goTopDepth(link_thumb);
        if (link_thumb._visible && links_num[0] > 0) {
            // マイリストみてる時
...
/* 使い方 _time("name"); 計測するスクリプト _timeEnd("name") */
function _time(data) {
...
// NGIDフラグたててカウント++function NGIDFlagOn(no)
{
...
createSquareBtn(command_bar, "clear_all", "全消", white12b_fmt, 5, 46, 15, 36, 24, 0x303030, 'コマンドクリア');
...
createSquareBtn(command_bar, "aspect_sd", "4:3", white12b_fmt, 70, 718, 15, 36, 24, 0x303030, '4:3の比率に変更');
command_bar.aspect_sd.onRelease = function() {
    changeAspect(4, 3);
    this._alpha = 40;
};
createSquareBtn(command_bar, "aspect_hd", "16:9", white12b_fmt, 71, 755, 15, 36, 24, 0x303030, '16:9の比率に変更');
command_bar.aspect_hd.onRelease = function() {
    changeAspect(16, 9);
    this._alpha = 40;
};
createSquareBtn(command_bar, "aspect_hd_cut", "16:9\ncut", white11b_fmt, 72, 792, 15, 36, 24, 0x303030, '16:9の比率に変更 (上下カット)');
command_bar.aspect_hd_cut.onRelease = function() {
    changeAspect(16, 9, true);
    this._alpha = 40;
};
//  全画面表示（最大横幅）
createSquareBtn(command_bar, "screen_full_side", "Wide", white12b_fmt, 73, 829, 15, 36, 24, 0x303030, '全画面化 (上下カット)');
command_bar.screen_full_side.onRelease = function() {
    changeAspect(16, 9, true);
    changeMonitorFull();
};
// 全画面表示
createSquareBtn(command_bar, "screen_full", "Full", white12b_fmt, 74, 866, 15, 36, 24, 0x303030, '全画面化 (通常比率)');
command_bar.screen_full.onRelease = function() {
    changeAspect();
    changeMonitorFull();
};
...
command_bar.smoothing.onRelease = function() {
    ...
    this._alpha = smoothing ? 60 : 80;
};
command_bar.smoothing.onRollOver = function() {
    clearInterval(nico.lineWindowShowIntervalId);
    nico.lineWindowShowIntervalId = setInterval(nico.showLineWindow, nico.lineWindowShowTime, 'スムージング切り替え');
    this._alpha = smoothing ? 60 : 80;
};
command_bar.smoothing.onRollOut = function() {
    clearInterval(nico.lineWindowShowIntervalId);
    nico.hideLineWindow();
    this._alpha = smoothing ? 40 : 100;
};
...
function showThumb() {
    if (!key_down_thumb && movie_started) {
        thumb_view._visible = false;
        thumb_text._visible = false;
        nico.video_base_video._alpha = 100;
    } else if (thumb_view != undefined && thumb_view.getBytesTotal() > 0) {
        thumb_view._xscale = 300;
        thumb_view._yscale = 300;
        if (nico.videowindow_height != undefined) {
            thumb_view._x = (nico.videowindow_width - thumb_view._width) / 2;
            thumb_view._y = (nico.videowindow_height - thumb_view._height) / 2;
        } else {
            thumb_view._x = (nico.videowindow._width - thumb_view._width) / 2;
            thumb_view._y = (nico.videowindow._height - thumb_view._height) / 2;
        }
        goPrevDepth(thumb_view, nico.videowindow.titleContainer);
        // サムネ用タイトル
        if (!video_title) {
            JS_getTitle();
            thumb_text = nico.videowindow.createTextField("thumb_title", nico.videowindow.getNextHighestDepth(), 0, 0, 1, 1);
            thumb_text.text = video_title;
            thumb_text.type = "dynamic";
            thumb_text.border = false;
            thumb_text.autoSize = false;
            thumb_text.wordWrap = true;
            thumb_text.background = false;
            thumb_text.selectable = false;
            thumb_text.setTextFormat(thumb_fmt);
            thumb_text._y = thumb_view._y + thumb_view._height;
            thumb_text._width = nico.videowindow_width;
            thumb_text._height = nico.videowindow_height - thumb_view._y - thumb_view._height - 2;
        }
        if (key_down_thumb) {
            thumb_view._alpha = 100;
            thumb_view._visible = true;
            thumb_text._visible = true;
            nico.video_base_video._alpha = 0;
            nico.videowindow.playButton._alpha = 0;
            nico.videowindow.titleContainer._alpha = 0;
            nico.videowindow.descriptionContainer._alpha = 0;
            nico.videowindow.loadingImage._visible = false;
        } else {
            thumb_text._visible = !movie_loaded;
            thumb_view._visible = true;
            nico.videowindow.loadingImage._visible = !nico.startedVideo;
            if (nico.startedVideo && (mouse_on_videowindow || mouse_on_playButton)) {
                thumb_view._alpha = 100;
                nico.videowindow.playButton._alpha = 100;
                nico.videowindow.titleContainer._alpha = 25;
                nico.videowindow.descriptionContainer._alpha = 25;
            } else {
                thumb_view._alpha = 50;
                nico.videowindow.playButton._alpha = 100;
                nico.videowindow.titleContainer._alpha = 100;
                nico.videowindow.descriptionContainer._alpha = 100;
            }
        }
    }
}
...
function thumbLoadTimer() {
    // 読み込み開始してなければ開始
    if (thumb_view == undefined) {
        if (nico.videowindow != undefined) {
            thumb_view = nico.videowindow.createEmptyMovieClip("thumb", 0);
            thumb_view._visible = false;
            thumb_view.loadMovie(thumbImage);
        }
        setTimeout(thumbLoadTimer, 500);
    } else if (thumb_view.getBytesTotal() > 0) {
        showThumb();
    } else if (!movie_started) {
        setTimeout(thumbLoadTimer, 500);
    }
}
// アスペクト比変更

function changeAspect(w, h, cut, initialize) {
    if (w && h) {
        video_aspect_ratio = w / h;
    } else {
        video_aspect_ratio = nico.player.videoStream_width / nico.player.videoStream_height;
    }
    if (initialize) {
        // 表示範囲用マスクの上書き
        nico.setNicowarSizeMask = function() {
            nico.video_base.setMask(nico.video_base.video_base_mask);
        };
        nico.setNicowarSizeMask();
        nico.video_base.video_base_mask._visible = false;
    }
    video_wide_cut = cut && nico.player.videoStream_width - nico.player.videoStream_height * w / h < -1;
    changeScale(video_screen_mode, initialize);
}
// 画面表示モード切り替え
var screen_mode = {
    normal: 0,
...
function changeScale(mode, initialize) {
    var video_w;
    var video_h;
    var video_frame_w;
    var video_frame_h;
    var video_frame_aspect_ratio;
    var space_side = nico.videowindow_width - nico.player.video_max_width;
    var space_top = Math.min(24, nico.player.video_max_height - nico.player.video_max_width / video_aspect_ratio);
    if (mode == screen_mode.normal) {
        video_frame_w = nico.player.video_max_width;
        video_frame_h = nico.player.video_max_height - space_top;
        video_frame_aspect_ratio = video_frame_w / video_frame_h;
        if (spread_wide_player) {
            video_frame_aspect_ratio = 16 / 9;
        }
    } else {
        if (mode == screen_mode.full) {
            video_frame_w = System.capabilities.screenResolutionX;
            video_frame_h = System.capabilities.screenResolutionY;
        } else {
            video_frame_w = nico.player.getStageWidth();
            video_frame_h = Math.min(video_frame_w * nico.player.video_max_height / nico.player.video_max_width, nico.player.getStageHeight());
        }
        if (transparent_inputarea) {
            video_frame_h -= nico.controller.controll_bar._height + nico.inputArea._height;
        }
        if (!hide_header && !transparent_header) {
            video_frame_h -= 58;
        }
        video_frame_aspect_ratio = video_frame_w / video_frame_h;
        video_frame_w = nico.player.video_max_height * video_frame_w / video_frame_h;
        video_frame_h = nico.player.video_max_height;
    }
    // 実際の表示範囲を算出
    if (video_frame_aspect_ratio > video_aspect_ratio) {
        video_w = video_frame_h * video_aspect_ratio;
        video_h = video_frame_h;
    } else {
        video_w = video_frame_h * video_frame_aspect_ratio;
        video_h = video_frame_h * video_frame_aspect_ratio / video_aspect_ratio;
    }
    // ビデオサイズ変更
    nico.video_base_video._xscale = video_w * 100 / video_base_w;
    nico.video_base_video._yscale = video_h * 100 / video_base_h;
    nico.video_base_video._x = (nico.player.video_max_width - video_w) / 2;
    nico.video_base_video._y = (nico.player.video_max_height - video_h) / 2;
    // 表示範囲の更新
    nico.video_base.video_base_mask._width = video_w;
    nico.video_base.video_base_mask._height = video_h;
    nico.video_base.video_base_mask._x = (nico.player.video_max_width - video_w) / 2;
    nico.video_base.video_base_mask._y = (nico.player.video_max_height - video_h) / 2;
    if (video_wide_cut) {
        var video_source_h = video_w * nico.player.videoStream_height / nico.player.videoStream_width;
        nico.video_base_video._yscale = video_source_h * 100 / video_base_h;
        nico.video_base_video._y = (nico.player.video_max_height - video_source_h) / 2;
    }
    if (mode == screen_mode.normal) {
        // マスク変更
        bg._width = Math.max(video_w, video_frame_w) + space_side;
        bg._height = Math.max(video_h, video_frame_h) + space_top;
        bg._x = (nico.videowindow_width - bg._width) / 2;
        bg._y = (nico.videowindow_height - bg._height) / 2;
        // プレイヤーの幅変更
        video_normal_width = bg._width;
        video_normal_offset = video_normal_width - nico.videowindow_width;
        // 75% までは縮小してなんとかしてみる
        var rscale = Math.max(75, (tabmenu_default_width - video_normal_offset) * 100 / tabmenu_default_width);
        nico.tabmenu._xscale = rscale;
        header._xscale = rscale;
        if (design_mode != 0) {
            main_bar._xscale = rscale;
        }
        flv_booster._xscale = rscale;
        nico.inputArea._x = nico.INPUTAREA_DEFAULT_X + video_normal_offset / 2;
        nico.CommandMenu._x = nico.INPUTAREA_DEFAULT_X + video_normal_offset / 2;
        nico.header._x = nico.HEADER_DEFAULT_X + video_normal_offset / 2;
        nico.tabmenu._x = tabmenu_default_x + video_normal_offset;
        header._x = nico.header._x + 550 + video_normal_offset / 2;
        main_bar._x = nico.header._x + design.normal_bar.x[design_mode] + (design_mode == 0 ? 0 : video_normal_offset / 2);
        flv_booster._x = header._x + 143 * rscale / 100;
        nico.waku._width = nico.tabmenu._x + tabmenu_default_width * rscale / 100 + 4;
        bottom_line._width = nico.waku._width;
        /*//// 縮めないやつ
		nico.inputArea._x = nico.INPUTAREA_DEFAULT_X + video_normal_offset / 2;
		nico.CommandMenu._x = nico.INPUTAREA_DEFAULT_X + video_normal_offset / 2;
		nico.header._x = nico.HEADER_DEFAULT_X + video_normal_offset / 2;
		nico.tabmenu._x = tabmenu_default_x + video_normal_offset;
		nico.waku._width = 950 + video_normal_offset;
		...
		bottom_line._width = nico.waku._width;
		////*/
        setPlayerScale(_root._xscale);
    } else {
        // マスク変更
        bg._width = Math.max(nico.videowindow_width, (video_base_w * nico.video_base_video._xscale / 100) + (nico.videowindow_width - nico.player.video_max_width));
        bg._height = nico.videowindow_height;
        if (change_bgcolor) {
            bg._width = Math.max(bg._width, nico.player.getStageWidth());
            bg._height = (nico.inputArea._y + nico.inputArea._height) * 100 / nico.player._mc._yscale;
        }
        bg._x = Math.floor((nico.videowindow_width - bg._width) / 2);
        bg._y = Math.floor((nico.videowindow_height - bg._height) / 2);
        if (change_bgcolor) {
    ...
    video_screen_mode = mode;
    // 自動スムージング設定
    setAutoSmoothing(initialize);
}
// 全画面表示

function changeMonitorFull() {
    // ロード前に最大化するとおかしくなる
    if (!movie_loaded) {
        return;
    }
    ...
    // バージョン9未満は不可
    if (detectFlashVer(9, 0, 18)) {
        key_down = false;
        mouse_down = false;
        key_down_thumb = false;
        if (Stage["displayState"] != "fullScreen") {
...
function setPlayerScale(scl) {
    if (nico.isLargeScreen) {
        return;
    }
    scl = Math.max(25, Math.min(1000, scl));
    //var w = ( root_base_w + video_normal_offset ) * scl / 100 + 5;
    var w = (root_base_w - 950 + nico.waku._width) * scl / 100 + 5;
    var h = root_base_h * scl / 100 + 5;
    _root._xscale = _root._yscale = scl;
    doJavaScript("$('flvplayer_container').style.width = $('flvplayer').style.width = " + w + "+'px';" + "$('flvplayer_container').style.height = $('flvplayer').style.height = " + h + "+'px';");
}
// コメントをローカルcgiにポストする
...
function sendLocalXML() {
    ...
    var fwNo = -1;
    while (fwNo == -1 && no <= last_no) {
        // めったにないけどnoが飛び番号で存在しなかった場合のループ
...
EventListener.onFullScreen = function(bFull) {
    if (!bFull && nico.isLargeScreen) {
        showMouse();
        showUI();
        changeScale(screen_mode.max);
    }
};
...
key_listener.onKeyDown = function() {
    ...
    if (target == null || target == "_level0.screen" || target == "_level0.bg" || target.substr(8, 16) == "nico.controller." || target.substr(8, 25) == "nico.tabmenu.relation_tab") {
        ...
        case 8:
            ...
            break;
        case 9:
            if (video_wide_cut || video_aspect_ratio != 16 / 9) {
                if (video_wide_cut) {
                    changeAspect();
                } else {
                    changeAspect(16, 9, true);
        ...
        case 107:
            ...
            break;
        case 107:
            setPlayerScale(25 * (Math.floor(_root._xscale / 25) + 1));
            break;
        case 109:
            setPlayerScale(25 * (Math.ceil(_root._xscale / 25) - 1));
            break;
        case 187:
            setPlayerScale(25 * (Math.floor(_root._xscale / 25) + 1));
            break;
    ...
    if (viewUI && (hideUI_flag || hideUI_id != undefined) && (timed_hide_header || timed_hide_inputarea)) {
        setHideUITimer();
    }
};
...
mouse_listener.onMouseWheel = function(delta, target) {
    if (test_mode) {
        if (Key.isDown(17)) {
    ...
    if (!mouse_on_drag_tabmenu && (hideUI_flag || hideUI_id != undefined) && (timed_hide_header || timed_hide_inputarea)) {
        setHideUITimer();
    }
    while (target != undefined) {
...
var ex_cm = false;
var shift_resize = false;
var resize_base_scale;
var resize_base_x;
var resize_base_y;
var old_x = _root._xmouse;
var old_y = _root._ymouse;
mouse_listener.onMouseMove = function() {
    var x = _root._xmouse;
    var y = _root._ymouse;
    var v = Math.max(Math.abs(x - old_x), Math.abs(y - old_y)) + 1;
    var mouseout = false;
    // マウス状態のチェック
    if (Stage["displayState"] != "fullScreen") {
        mouse_down = mouse_down && Key.isDown(1);
        mouseout = x < v || x > Stage.width - v || y < v || y > Stage.height - v;
    }
    // コンテキストメニュー操作後チェック
    if (mouse_on_context_menu && contextCheck_id == undefined) {
        contextCheck_id = setTimeout(contextCheckOff, 500);
    }
    // マーキー表示時はヘッダが表示されててもバーを隠してる
    if (nico.isLargeScreen && (timed_hide_header || timed_hide_inputarea || (design_mode != 0 && !hide_header))) {
        showMouse();
        hideMouse_id = setTimeout(hideMouse, 1000);
        // UI変更に付きHeaderとMain_barの両方で判定
        if (!force_hideUI) {
            // 上下端にマウスがあるときにだけ表示
            if (!mouseout && (((timed_hide_header || (design_mode != 0 && !hide_header)) && y <= header._y + header._height) || ((timed_hide_header || (design_mode != 0 && !hide_header)) && y <= main_bar._y + main_bar._height) || (timed_hide_inputarea && y >= nico.controller._y))) {
                showUI();
            } else {
    ...
    // 画面拡大時にコメントリストを出すテスト
    mouse_on_drag_tabmenu = false;
    if (nico.isLargeScreen && (mouse_drag_tabmenu || timed_hide_commentlist)) {
        showMouse();
        hideMouse_id = setTimeout(hideMouse, 1000);
        if (force_hideUI) {
            nico.tabmenu._visible = false;
            if (mouse_drag_tabmenu) {
                mouse_drag_tabmenu = false;
                nico.tabmenu.stopDrag();
            }
        } else {
            // 画面外を擬似的に判断
            if (!mouse_drag_tabmenu && mouseout) {
                nico.tabmenu._visible = false;
            }
            // 右端にマウスがあるときにだけ表示
            else if (!nico.tabmenu._visible && x > Stage.width - 32) {
                showMouse();
                var min_tabmenu = nico.controller._y - tabmenu_default_height;
                if (!clip_height) {
                    min_tabmenu -= 16;
                }
                nico.tabmenu._visible = true;
                nico.tabmenu._x = Stage.width - tabmenu_default_width;
                nico.tabmenu._y = Math.max(0, Math.min(min_tabmenu, _ymouse - nico.tabmenu.loglist_menu._y * 5 / 4));
                mouse_on_drag_tabmenu = true;
                goNextDepth(nico.tabmenu, nico.controller);
            } else if (nico.tabmenu._visible && (mouse_drag_tabmenu || nico.tabmenu.hitTest(x, y, true))) {
                mouse_on_drag_tabmenu = true;
            } else if (x < Stage.width - 48) {
    ...
    // コメントに合わせてスクロール待ち
    if (playScroll_id != undefined) {
        clearInterval(playScroll_id);
        if (nico.tabmenu._visible && nico.LogList.hitTest(x, y, true)) {
            playScroll_id = setTimeout(playScrollJump, 1000);
        } else {
            playScrollJump();
        }
    }
    // マウスによるリサイズ
    if (mouse_down && !nico.isLargeScreen) {
        if (shift_resize && !Key.isDown(Key.SHIFT)) {
            shift_resize = false;
            mouse_drag_resize_x = false;
            mouse_drag_resize_y = false;
        }
        if (mouse_drag_resize_x && (!mouse_drag_resize_y || x / resize_base_x < y / resize_base_y)) {
            click_valid = false;
            setPlayerScale(x * _root._xscale * resize_base_scale / resize_base_x);
        } else if (mouse_drag_resize_y) {
            click_valid = false;
            setPlayerScale(y * _root._yscale * resize_base_scale / resize_base_y);
        }
    }
    old_x = x;
    old_y = y;
...
mouse_listener.onMouseDown = function() {
    ...
    mouse_down = true;
    // マウスによるリサイズ
    if (drag_resize && !nico.isLargeScreen && (!pref_menu._visible || !pref_menu.hitTest(x, y, true)) && (!ngid_menu._visible || !ngid_menu.hitTest(x, y, true)) && (!link_thumb._visible || !link_thumb.hitTest(x, y, true)) && (!command_bar._visible || !command_bar.hitTest(x, y, true)) && (!kakikomi_menu._visible || !kakikomi_menu.hitTest(x, y, true)) && (!end_time_input._visible || !end_time_input.hitTest(x, y, true))) {
        if (Key.isDown(Key.SHIFT)) {
            shift_resize = true;
            if (root_base_w / 2 <= _root._xmouse && root_base_h / 2 <= _root._ymouse) {
                mouse_drag_resize_x = true;
                mouse_drag_resize_y = true;
            } else {
                mouse_drag_resize_x = _root._xmouse / root_base_w > _root._ymouse / root_base_h;
                mouse_drag_resize_y = !mouse_drag_resize_x;
            }
        } else {
            shift_resize = false;
            mouse_drag_resize_x = root_base_w <= x;
            mouse_drag_resize_y = root_base_h <= y;
        }
        resize_base_x = x * _root._xscale;
        resize_base_y = y * _root._yscale;
        resize_base_scale = _root._xscale;
    } else {
        mouse_drag_resize_x = false;
        mouse_drag_resize_y = false;
    }
    // コメントリストのドラッグ
    if (mouse_on_drag_tabmenu && !nico.LogList.hitTest(x, y, true)) {
        mouse_drag_tabmenu = true;
        nico.tabmenu.startDrag();
    }
};
...
function relativeSeek(mode) {
    if (timeLine.check_seek.onEnterFrame == undefined) {
        ...
        if (mode == 'forward') {
            ...
            var xm = nico.videowindow._xmouse;
            var ym = nico.videowindow._ymouse;
            if (484 < xm && xm < nico.player.getStageWidth() && 193 < ym && ym < nico.player.getStageHeight()) {
...
function contextCheckOff() {
    mouse_on_context_menu = false;
    contextCheck_id = undefined;
}
function playScrollJump() {
    if (nico.LogList_menu.playScrollCheck.selected || nico.LogList_menu.waitCommentCheck.selected) {
        if (nico.tabmenu._visible && (mouse_down || !movie_loaded || nico.player.isPausing)) {
            playScroll_id = setTimeout(playScrollJump, 1000);
            return;
        }
        if (!nico.isLoglistAutoDL && nico.LogList_menu.waitCommentCheck.selected) {
            nico.waitCommentCheckListener.click();
        }
        if (!nico.isPlayScroll && nico.LogList_menu.playScrollCheck.selected) {
            var idx = nico.LogList.selectedIndex;
            nico.playScrollCheckListener.click();
            nico.LogList.selectedIndex = idx;
        }
    }
    playScroll_id = undefined;
}
function showMouse() {
...
function hideUI() {
    ...
    if (nico.isLargeScreen && !hideUI_flag) {
        ...
        var w = nico.player.getStageWidth();
        var h = nico.player.getStageHeight() - nico.player.VIDEOWINDOW_LARGE_DEFAULT_Y - nico.player.INPUTAREA_LARGE_HEIGHT;
        if ((timed_hide_inputarea || timed_hide_inputarea) && nico.controller._visible && !push_out_inputarea) {
    ...
    if (nico.isLargeScreen) {
        if (hideUI_flag) {
            ...
            if (!nico.controller._visible) {
                ...
                nico.inputArea._alpha._y = Stage.height - nico.inputArea._height - 10;
                if (cand_ng_id.length > 0) {
                    nico.tabmenu.loglist_menu.add_id._visible = true;
                }
                if (transparent_inputarea) {
...
function replaceFunction() {
    ...
    // 実際のジャンプ処理
    nico.jumpVideo = function(id, from, length, return_, returnmsg, time, newwindow, jumphash) {
        if (id == undefined || id == nico.jumpWindowFlag) {
            if (id == undefined) {
                nico.jumpWindowFlag = NaN;
            }
            return false;
        }
        nico.jumpWindowFlag = id;
        if (nico.thumbWatch) {
            if (!nico.fv_no_jump_msg) {
                showWindow(getSystemMessage('jump()の実行'), 'scriptPlayerDialog', getSystemMessage('外部プレイヤーモードではジャンプできません。'));
            }
            return false;
        }
        if (nico.owner_thread_edit_mode) {
            showWindow(getSystemMessage('jump()の実行'), 'scriptPlayerDialog', getSystemMessage('[[id]]へのジャンプイベントが発生しましたが、スクリプト編集中のためキャンセルしました。', id));
            return false;
        }
        var tossObj = jumper.tossedObject;
        if (tossObj && tossObj.jumphash == jumphash) {
            tossObj.jumphash = null;
            return false;
        }
        nico.playing = nico.player.state == 'playing';
        nico.playerPause(true);
        var params = {};
        ...
        if (return_) {
            ...
            if (returnmsg !== undefined) {
                params.returnmsg = returnmsg;
            }
            params.returnto = time;
            obj.jumphash = jumphash;
        }
        if (check_autojump) {
            nico.playerPause(true);
            if (!auto_link) {
                var cand_links = searchMessage("@ジャンプ " + id);
            ...
            link_thumb.load.onRelease();
            link_thumb.onEnterFrame = function() {
                click_valid = false;
                if (!link_thumb._visible || !nico.player.isPausing || key_down || (mouse_down && !link_thumb.hitTest(_root._xmouse, _root._ymouse, true))) {
                    link_thumb._visible = false;
                    nico.drawSystemText(undefined);
                    nico.playerPause(false);
                    link_thumb.onEnterFrame = undefined;
                }
            };
            if (!link_thumb._visible) {
                link_thumb._x = (Stage.width - link_thumb._width) / 2;
                link_thumb._y = (Stage.height - link_thumb._height) * 4 / 5;
                link_thumb._visible = true;
            }
            goTopDepth(link_thumb);
        } else {
            setTimeout(util.ObjectUtils.bind(function() {
                nico.jumper.jump(id, params, obj, newwindow);
                if (newwindow) {
                    nico.drawSystemText(undefined);
                }
            },
    ...
    // ジャンプ時のテキスト表示
    nico.drawSystemText = function(str) {
        var mc = nico.getSystemMsgLayer().text_mc;
        if (!str) {
            mc._visible = false;
            mc[nico.mcName]._x = 0;
            mc[nico.mcName]._y = 0;
            mc[nico.mcName]._width = 1;
            mc[nico.mcName]._height = 1;
            return undefined;
        }
        with(mc) {
            mc.textField.text = str;
            var fmt = mc.textField.getTextFormat();
            fmt.size = 32;
            fmt.color = 0;
            fmt.font = 'Arial';
            mc.textField.setTextFormat(fmt);
        }
        mc.textField._x = (nico.player.video_max_width - mc.textField._width) / 2;
        mc.textField._y = (nico.player.video_max_height - mc.textField._height) / 2;
        mc[nico.mcName]._x = -System.capabilities.screenResolutionX;
        mc[nico.mcName]._y = -System.capabilities.screenResolutionY;
        mc[nico.mcName]._width = System.capabilities.screenResolutionX * 2;
        mc[nico.mcName]._height = System.capabilities.screenResolutionY * 2;
        mc._visible = true;
    };
    ...
    // コメントリストをスクロールしてもチェックを外さない
    nico.LogListListener.scroll = function(evt) {
        if (evt.target == nico.LogList_community) {
            nico.MAX_LOGLIST_LEN = nico.MAX_LOGLIST_COMMUNITY_LEN;
        }
        // 一時的にスクロールと非同期化
        if (!nico.mm) {
            if (evt.target.length - nico.MAX_LOGLIST_LEN < MAX_LOGLIST_LEN) {
                if (nico.LogList_menu.waitCommentCheck.selected) {
                    if (playScroll_id != undefined) {
                        clearInterval(playScroll_id);
                    }
                    nico.isLoglistAutoDL = false;
                    nico.isPlayScroll = false;
                    playScroll_id = setTimeout(playScrollJump, 1000);
                }
            }
        }
        if (nico.LogList_menu.playScrollCheck.selected) {
            if (playScroll_id != undefined) {
                clearInterval(playScroll_id);
            }
            nico.isLoglistAutoDL = false;
    ...
    // 位置調整関数が変わった使われ方をしているので補正
    nico.setPositionNoScale = function(cmc, pmc, proc) {
        var v2 = pmc._xscale;
        var v3 = pmc._yscale;
        pmc._xscale = 100;
        pmc._yscale = 100;
        if (pmc == nico.videowindow) {
            // bgを借用
            var org_w = nico.videowindow.bg._width;
            var org_h = nico.videowindow.bg._height;
            proc(cmc, nico.videowindow.bg);
            nico.videowindow.bg._width = org_w;
            nico.videowindow.bg._height = org_h;
        } else {
            proc(cmc, pmc);
        }
        pmc._xscale = v2;
    ...
    // 黒背景を置き換える
    // ビデオウインドウの最奥背景は自動命名されているので直接取得できない
    // 深さで取得してみるが、おかしくなるようならこのfor文ごと消す
    ////
    for (v1 in nico.videowindow) {
        var mc = nico.videowindow[v1];
        if (mc instanceof MovieClip) {
            if (bg == undefined || mc.getDepth() < bg.getDepth()) {
                bg = mc;
            }
        }
    }
	...
	if( bg == undefined )
	{
		bg = nico.videowindow.createEmptyMovieClip( 'bg', nico.videowindow.getNextHighestDepth() );
		nico.fill_rect( bg, 0, 0, nico.videowindow_width, nico.videowindow_height, 0 );
		goBottomDepth( bg );
	}
	////*/
    // ビデオウインドウ用マスク作成
    nico.videowindow.Overlay.setMask(null);
    nico.videowindow.mask._visible = false;
    nico.videowindow.mask.clear();
    nico.videowindow.mask.removeMovieClip();
    nico.videowindow.mask = bg.createEmptyMovieClip('mask', bg.getNextHighestDepth());
    nico.fill_rect(nico.videowindow.mask, 0, 0, bg._width, bg._height, 0);
    nico.videowindow.setMask(nico.videowindow.mask);
    // 表示範囲用マスク
    if (!nico.isShiSwfPlayer()) {
        nico.video_base.createEmptyMovieClip('video_base_mask', 10000);
        nico.fill_rect(nico.video_base.video_base_mask, 0, 0, nico.video_width, nico.video_height, 0);
        nico.video_base.setMask(nico.video_base.video_base_mask);
    }
    // 再生中に再生ボタンを出さない
    ...
    nico.getBgms = function() {
        nico._getBgms();
        if (nico.videowindow.playButton._visible && !nico.player.isPausing) {
            nico.videowindow.playButton._visible = false;
        }
    };
    // スクリーンクリック用
    ...
    bg.onPress = function() {
        var now = getTimer();
        var x = _root._xmouse * _root._xscale / 100;
        var y = _root._ymouse * _root._yscale / 100;
        double_click_valid = false;
        if (!mouse_on_drag_tabmenu && !mouse_on_context_menu && (Key.isDown(1) || Stage["displayState"] == "fullScreen")) {
            if (now - click_time < 300 && Math.abs(x - click_x) < 5 && Math.abs(y - click_y) < 5) {
                double_click_valid = click_valid;
            }
            click_valid = true;
        } else {
            click_valid = false;
    ...
    // スクリーン上のクリック用
    bg.onRelease = function() // ※一部環境で不具合が起きるのでonReleaseに変更
    {
        if (click_valid) {
            ...
            // ダブルクリック判定
            if (double_click_valid && (double_click_fullscreen || double_click_fullmonitor)) {
                if (nico.isLargeScreen || Stage["displayState"] == "fullScreen") {
                    nico.controller.controller_submenu.NormalScreenButton.onRelease();
                } else if (Key.isDown(Key.SHIFT) ^ double_click_fullmonitor) // shift押しで全画面（隠しショートカット）
                {
                    changeMonitorFull();
                }
                else {
            ...
            click_id = false;
            // シングルクリック
            if (movie_started && !owner_thread_edit_mode) {
                if (nico.selectTab != nico.TAB_WAYBACK) {
                    var id = searchID();
                    click_id = list_mode == "cand_ng_id" || 0 <= id;
                    emphID(id);
                    updateLogList("cand_ng_id");
                }
            } else {
    ...
    if (nico.coordinatesLayer) {
        nico.coordinatesLayer.onPress = nico.videowindow.onPress;
    }
    ...
    nico.inputArea.ChatInput1.dispatchEvent = function(evt) {
        ...
        if (evt.type == 'enter') {
            ...
            if (!kill_enter) {
                ...
                if (Key.isDown(17) && Key.isDown(13)) {
                    // Ctrl+Enter
                    nico.inputArea.ChatSendButton.onRelease();
                }
            }
    ...
    createSquareBtn(nico.tabmenu.loglist_menu, "add_id", "NGIDに追加", white12b_fmt, 1, 315, 14, 90, 23, 0x303030);
    nico.tabmenu.loglist_menu.add_id.onRelease = function() {
        this._visible = false;
        cand_ng_id = deleteRepField(cand_ng_id, "user_id", false);
        updateFilter("add_id");
    };
    ...
    createSquareBtn(nico.tabmenu.loglist_menu, "normal_list", "一覧に戻る", white12b_fmt, 2, 198, 14, 90, 23, 0x500000);
    nico.tabmenu.loglist_menu.normal_list.onRelease = function() {
        updateLogList("normal");
    };
    nico.tabmenu.loglist_menu.normal_list._visible = false;
    // 強調表示ボタン リストのあたりに表示する3
    createSquareBtn(nico.tabmenu.loglist_menu, "cand_ng_id_list", "IDで抽出", white12b_fmt, 3, 198, 14, 90, 23, 0x303050);
    nico.tabmenu.loglist_menu.cand_ng_id_list.onRelease = function() {
        updateLogList("cand_ng_id");
    };
    nico.tabmenu.loglist_menu.cand_ng_id_list._visible = false;
    ...
    nico.Connection.onCMsgGetThreadResult = function(resultcode, thread_id, tmp_last_res, ticket, message, locked, revision, offset, fork, newServerTime, ButtonIgnored, clickRev) {
        ...
        if (resultcode == 0) {
            // 定数。元はNiwavidePacket.THREAD_FOUND
    ...
    nico.controller.controller_submenu.LargeScreenButton.onRelease = function() {
        ...
        // nico.headerを動画の前にする
        sortDepth1(nico.videowindow, nico.header);
        ...
        // シークバーを動画の前にする
        sortDepth1(nico.videowindow, nico.controller);
        // 最大化サイズに動画を引き延ばす
        changeScale(screen_mode.max);
    };
    // 最大化から元に戻す処理
    ...
    nico.controller.controller_submenu.NormalScreenButton.onRelease = function() {
        ...
        // 最大化からサイズを戻す
        changeScale(screen_mode.normal);
        Stage["displayState"] = "normal";
        main_bar._visible = true;
        header._visible = true;
        mouse_drag_tabmenu = false;
        nico.tabmenu.stopDrag();
        nico.controller._x = nico.player.CONTROLLER_DEFAULT_X;
        nico.controller._y = nico.player.CONTROLLER_DEFAULT_Y;
        nico.inputArea._x = nico.INPUTAREA_DEFAULT_X + video_normal_offset / 2;
        ...
        nico.header.bgm_marquee_base._y = nico.header.marquee_base._y;
        nico.tabmenu._x = tabmenu_default_x + video_normal_offset;
        nico.tabmenu._y = tabmenu_default_y;
        if (clip_height) {
            nico.tabmenu._y += 2;
        }
        nico.tabmenu._visible = true;
        nico.tabmenu._alpha = 100;
        header._x = nico.header._x + 550 + video_normal_offset / 2;
        ...
        ngid_menu._x = 552 + video_normal_offset;
        ngid_menu._y = 85;
        link_thumb._x = 552 + video_normal_offset;
        link_thumb._y = 277;
        pref_menu._x = 561 + video_normal_offset;
        pref_menu._y = 75;
        bottom_line._visible = true;
        flv_booster._visible = true;
        // シークバーは動画のうしろに
        sortDepth2(nico.header, nico.videowindow);
        sortDepth2(nico.controller, nico.videowindow);
        // マーキー表示中なら表示
        if (autopop_mode) {
            autopop_mode = false;
            Show_marquee();
    ...
    nico.stageListener.onResize = function() {
        if (nico.isLargeScreen) {
            // 最大化時のレイアウトを再設定
            if (push_out_inputarea) {
                now_push_out_inputarea = true;
                doJavaScript("document.body.style.height = ( document.documentElement.clientHeight + " + (nico.controller.controll_bar._height + nico.inputArea._height) + ") * 100 / document.documentElement.clientHeight + '%';");
            } else if (now_push_out_inputarea) {
                now_push_out_inputarea = false;
                flash.external.ExternalInterface.call('maximizePlayer');
                return;
            }
            nico.player.VIDEOWINDOW_LARGE_DEFAULT_Y = 58;
            if (hide_header) {
            ...
            if (transparent_commentlist) {
                nico.tabmenu._alpha = 100 - inputarea_alpha;
            } else {
                nico.tabmenu._alpha = 100;
            }
            var w = nico.player.getStageWidth();
            var h = nico.player.getStageHeight() - nico.player.VIDEOWINDOW_LARGE_DEFAULT_Y;
            if (!transparent_inputarea) {
                h -= nico.controller.controll_bar._height + nico.inputArea._height;
            }
            var r = Math.min(w / nico.player.video_max_width, h / nico.player.video_max_height);
            nico.videowindow._x = 0;
            nico.header._x = Math.floor((nico.player.getStageWidth() - nico.header._width) / 2);
            nico.inputArea._x = Math.floor((nico.player.getStageWidth() - nico.inputArea._width) / 2);
            nico.inputArea._y = Math.floor(nico.player.VIDEOWINDOW_LARGE_DEFAULT_Y + nico.videowindow_height * r - nico.inputArea._height);
            if (!transparent_inputarea) {
                nico.inputArea._y += nico.controller.controll_bar._height + nico.inputArea._height;
            }
            nico.CommandMenu._x = nico.inputArea._x;
            nico.CommandMenu._y = nico.inputArea._y - nico.CommandMenu._height;
            nico.player.setLsize(true);
            nico.player._mc._xscale = r * 100;
            nico.player._mc._yscale = r * 100;
            nico.player._mc._x = (w - nico.player.VIDEOWINDOW_DEFAULT_WIDTH * r) / 2;
            nico.player._mc._y = nico.player.VIDEOWINDOW_LARGE_DEFAULT_Y;
            changeScale(video_screen_mode);
            nico.controller._x = Math.max(0, (nico.player.getStageWidth() - nico.controller._width) / 2);
            nico.controller._y = nico.inputArea._y - nico.controller.controll_bar._height;
            command_bar._x = Math.max(0, (nico.player.getStageWidth() - command_bar._width) / 2);
            command_bar._y = nico.controller._y - command_bar._height;
            main_bar._x = nico.header._x + design.normal_bar.x[design_mode];
            header._x = nico.header._x + 550;
            flv_booster._x = header._x + 143;
            if (transparent_commentlist) {
                nico.tabmenu._x = nico.player.getStageWidth() - tabmenu_default_width;
                nico.tabmenu._y = nico.controller._y - tabmenu_default_height;
                if (!clip_height) {
                    nico.tabmenu._y -= 16;
                }
                sortDepth1(nico.controller, nico.tabmenu);
            }
            showMouse();
            hideMouse_id = setTimeout(hideMouse, 1000);
            showUI();
            if (timed_hide_timelimit <= 0) {
                ...
                setHideUITimer();
                nico.tabmenu._visible = transparent_commentlist;
            }
            ngid_menu._x = (nico.player.getStageWidth() - ngid_menu._width) * 15 / 16;
            ngid_menu._y = (nico.player.getStageHeight() - ngid_menu._height) / 8;
            link_thumb._x = (nico.player.getStageWidth() - link_thumb._width) * 15 / 16;
            link_thumb._y = (nico.player.getStageHeight() - link_thumb._height) / 4;
            pref_menu._x = (nico.player.getStageWidth() - pref_menu._width) * 15 / 16;
            pref_menu._y = (nico.player.getStageHeight() - pref_menu._height) / 8;
        } else {
            nico.player.setLsize(false);
            //if( ngid_menu._visible )
            //{
            ngid_menu._x = Math.min(Stage.width - ngid_menu._width, ngid_menu._x);
            ngid_menu._y = Math.min(Stage.height - ngid_menu._height, ngid_menu._y);
            //}
            //else
            //{
            //	ngid_menu._x = 552 + video_normal_offset;
            ...
            //if( link_thumb._visible )
            //{
            link_thumb._x = Math.min(Stage.width - link_thumb._width, link_thumb._x);
            link_thumb._y = Math.min(Stage.height - link_thumb._height, link_thumb._y);
            //}
            //else
            //{
            //	link_thumb._x = 552 + video_normal_offset;
            ...
            //if( pref_menu._visible )
            //{
            pref_menu._x = Math.min(Stage.width - pref_menu._width, pref_menu._x);
            pref_menu._y = Math.min(Stage.height - pref_menu._height, pref_menu._y);
            //}
            //else
            //{
            //	pref_menu._x = 561 + video_normal_offset;
        ...
        if (evt.type == 'headerRelease') {
            ...
            if (v1.selectedItem != undefined) {
                if (list_mode == "normal") {
                    ...
                    nico.tabmenu.loglist_menu.cand_ng_id_list._visible = true;
    ...
    nico.Connection.onCMsgChat = function(message, thread, no, vpos, date, mail, name, yourpost, user, raw_user, deleted, premium, anonymity, fork) {
        var v4;
        if (fork == 1) {
            // 定数。元はNiwavidePacket.FORK_OWNER
            v4 = 'game';
        } else {
        ...
        if (nico.o.ng_up != undefined) {
            ...
            } else if (pass_through_message_filter) {
                // フィルタを透過
    ...
    nico.o.onLoad = function(success) {
        ...
        if (nico.forceNicowariOff) {
            nico.System_menu.marqueeSkipCheckBox.selected = true;
            nico.System_menu.marqueeSkipCheckBox.enabled = false;
        }
        if (!e && !mm && !owner_thread_edit_mode) {
            nico.enableTab(nico.tabmenu.wayback_tab);
        }
        ...
        if (!nico.o.user_id && nico.o.closed) {
            ...
            if (nico.o.l != undefined && nico.o.url != undefined) {
                ...
                this);
                if (!v9) {
                    // TubePlayerでutil.ObjectUtils.bindが動かない
    ...
    // 注意用ダイアログ作成
    attentionDialog = nico.attachMovie('dialog', "attentionDialog", nico.getNextHighestDepth());
    attentionDialog.closeButton._visible = false;
    attentionDialog._visible = false;
    attentionDialog.textFormat = new TextFormat();
    attentionDialog.textFormat.bold = true;
    attentionDialog.text.html = false;
    attentionDialog.title.setNewTextFormat(attentionDialog.textFormat);
    attentionDialog.title.text = '注意';
    // 最上位になるようロールオーバーウインドウの作り直し
    nico.lineWindow.removeMovieClip();
    nico.lineWindow = _root.createTextField('lineWindow_text', _root.getNextHighestDepth(), 0, 0, 0, 0);
    nico.lineWindow.autoSize = true;
    nico.lineWindow.type = 'dynamic';
    nico.lineWindow.background = true;
    nico.lineWindow.backgroundColor = 0xFFFF99;
    nico.lineWindow.border = true;
    nico.lineWindow.borderColor = 0x000000;
    nico.lineWindow._visible = false;
    nico.lineWindow.setNewTextFormat(nico.lineWindow_fmt);
    nico.setLineWindowPos = function() {
        nico.lineWindow._x = Math.min(_root._xmouse + 10, Stage.width * 100 / _root._xscale - nico.lineWindow._width - 3);
        nico.lineWindow._y = Math.min(_root._ymouse + 10, Stage.height * 100 / _root._yscale - nico.lineWindow._height - 3);
        goTopDepth(nico.lineWindow);
    };
    // ロールオーバーウィンドウの待ち時間を短くする
    nico.lineWindowShowTime = 200;
    /*↓↓ 実行されなくなったので元に戻す
	 //@CM・@BGMがある場合、getBgmsでopenPlayerFuncが実行されるので独立させた改編部detaModify()を追加して上
	書き
	 nico._getBgms = nico.getBgms;
	 nico.getBgms = function() {
	 nico._getBgms();
    ...
    nico.outAdAndplay = function(noPlay) {
        ...
        // 再生時開始最大化
        if (first_time_full || first_time_fullmonitor) {
            first_time_full = false;
            first_time_fullmonitor = false;
            if (nico.isLargeOK) {
                // 最大化状態でない
                if (!nico.isLargeScreen) {
                    if (first_time_fullmonitor_so.data.flag) {
                        // Flashの仕様で自動フルスクリーンはできない（エラーになる）
                        if (nico.adPlay) {
                            nico.playerPause(true);
                            showAttentionDialog("自動処理による全画面化はFlashで制限されています", function() {
                                changeMonitorFull();
                                nico.playerPause(false);
                            },
    ...
    // 自動再生のチェックBOX  	nico.autoPlayCheckListener.click = function()
    {
    ...
    if (nico.videowindow.getDepth() < nico.controller.getDepth()) {
        sortDepth2(nico.DeleteLabel, nico.videowindow);
        sortDepth2(nico.controller, nico.videowindow);
    }
    ...
    // タブメニューの座標を保存
    tabmenu_default_x = nico.tabmenu._x;
    tabmenu_default_y = nico.tabmenu._y;
    tabmenu_default_width = command_bar._x + command_bar._width - tabmenu_default_x;
    tabmenu_default_height = nico.tabmenu.icon_buttonok_off._y - 3;
    // なんとなくバーを太く
    if (wide_seek_bar) {
        nico.controller.seek_bar._height += 6;
        nico.controller.seek_bar._y -= 3;
    ...
    //ロード前に最大化するとおかしくなる
    nico.controller.controller_submenu.LargeScreenButton.enabled = false;
    //動画の基準サイズ保存
    video_base_w = nico.video_base_video._width * 100 / nico.video_base_video._xscale;
    video_base_h = nico.video_base_video._height * 100 / nico.video_base_video._yscale;
    video_normal_width = nico.videowindow_width;
    // 最初はサムネイルだけ表示
    nico.video_base_video._alpha = 0;
    ...
    // プレイヤの基準サイズ保存
    root_base_w = nico.waku._width;
    root_base_h = bottom_line._y + bottom_line._height;
    // 足りないときはのばす
    if (nico.waku._height < root_base_h) {
        nico.waku._height = root_base_h;
    }
    // 余計な部分を削除
    nico.waku.createEmptyMovieClip('mask', 10000);
    nico.waku.mask.beginFill(0);
    nico.waku.mask.moveTo(0, 0);
    nico.waku.mask.lineTo(root_base_w, 0);
    nico.waku.mask.lineTo(root_base_w, root_base_h);
    nico.waku.mask.lineTo(0, root_base_h);
    nico.waku.mask.endFill();
    nico.waku.setMask(nico.waku.mask);
    nico.tabmenu.createEmptyMovieClip('mask', 10000);
    nico.tabmenu.mask.beginFill(0);
    nico.tabmenu.mask.moveTo(0, 0);
    nico.tabmenu.mask.lineTo(tabmenu_default_width, 0);
    nico.tabmenu.mask.lineTo(tabmenu_default_width, tabmenu_default_height);
    nico.tabmenu.mask.lineTo(0, tabmenu_default_height);
    nico.tabmenu.mask.endFill();
    nico.tabmenu.setMask(nico.tabmenu.mask);
    if (clip_height) {
        nico.tabmenu._height -= 16;
        nico.tabmenu._y += 2;
    }
    ...
    if (e) {
        ...
        nico.tabmenu.loglist_menu.normal_list._x -= 20;
        nico.tabmenu.loglist_menu.cand_ng_id_list._x -= 20;
    }
    //if(test_mode){
    if (!is_video_owner) {
    ...
    if (useswfversion >= 7) {
        ...
        videowindowCM.customItems.push(screen_max);
        ...
        videowindowCM.customItems.push(cmi_aspect_16_9_cut);
        ...
        videowindowCM.customItems.push(cmi_switch_no_add_id_overlay);
        ...
        screen_max.visible = true;
        screen_normal.visible = true;
        cmi_aspect_4_3.visible = true;
        cmi_aspect_16_9.visible = true;
        cmi_aspect_16_9_cut.visible = true;
        videowindowCM.onSelect = function(obj, menu) {
            var is_play = !nico.player.isPausing;
            var w = (video_base_w * nico.video_base_video._xscale / 100);
            var h = (video_base_h * nico.video_base_video._yscale / 100);
            cmi_play.enabled = !is_play && movie_loaded;
            cmi_play.visible = !is_play || !movie_loaded;
            cmi_pause.enabled = is_play && movie_loaded;
            cmi_pause.visible = is_play && movie_loaded;
            cmi_smoothing_on.enabled = !smoothing && !nico.isShiSwfPlayer();
            cmi_smoothing_off.enabled = smoothing && !nico.isShiSwfPlayer();
            // 通常表示
            if (!nico.isLargeScreen) {
                screen_full.enabled = movie_loaded;
                screen_max.enabled = movie_loaded;
                screen_normal.enabled = false;
            } else {
                // 全画面表示
                if (Stage["displayState"] == "fullScreen") {
                    screen_full.enabled = false;
                    screen_max.enabled = true;
                }
                // 最大表示
                else {
            ...
            if (movie_loaded) {
                var org_w = nico.player.videoStream_width;
                var org_h = nico.player.videoStream_height;
                var w = (video_base_w * nico.video_base_video._xscale / 100);
                var h = (video_base_h * nico.video_base_video._yscale / 100);
                cmi_aspect_original.enabled = video_wide_cut || Math.abs(w - h * org_w / org_h) > 1;
                cmi_aspect_4_3.enabled = video_wide_cut || Math.abs(w - h * 4 / 3) > 1;
                cmi_aspect_16_9.enabled = video_wide_cut || Math.abs(w - h * 16 / 9) > 1;
                cmi_aspect_16_9_cut.enabled = !video_wide_cut && org_w - org_h * 16 / 9 < -1;
            } else {
                cmi_aspect_original.enabled = false;
                cmi_aspect_4_3.enabled = false;
                cmi_aspect_16_9.enabled = false;
            ...
            cmi_switch_add_id_overlay.enabled = !add_id_overlay;
            cmi_switch_add_id_overlay.visible = !add_id_overlay;
            cmi_switch_no_add_id_overlay.enabled = add_id_overlay;
            cmi_switch_no_add_id_overlay.visible = add_id_overlay;
            // コンテキストメニュー操作後チェック
            if (contextCheck_id != undefined) {
                clearInterval(contextCheck_id);
                contextCheck_id = undefined;
            }
            mouse_on_context_menu = true;
            // コンテキストメニューでカーソルを表示
            showMouse();
        };
    ...
    nico.NGMessage = function(msgs, slots, list, index, flag) {
        ...
        if (v1._backup != undefined) {
            ...
            // ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
            if (add_id) {
                // add_idならeditFieldする前にメッセージにID付与
                if (msgs == nico.Messages) {
...
function dataModify() {
    // 初期値を設定
    if (!nico.player.videoStream_width || !nico.player.videoStream_height) {
        nico.player.videoStream_width = 512;
        nico.player.videoStream_height = 384;
    }
    // 動画のメタデータを受け取ったときの処理
    nico.player.stream_ns.onMetaData = function(infoObject) {
        ...
        if (video_info == undefined) {
            ...
            showClockInfo(clock_mode);
            // 動画サイズを初期化
            changeAspect(nico.player.videoStream_width, nico.player.videoStream_height, false, true);
        }
    };
    ...
    nico.player._play_btn.onRelease = function() {
        //if( nico.player.isRealEnd || !nico.isInitBGM && rt.bgms )
        if (!nico.isInitBGM && nico.bgms) //末尾でも再生を許可
        {
            return undefined;
        }
        nico.player.pause();
        if (nico.player._rt.focusManager) {
            nico._focusrect = false;
    ...
    nico.player.dispatchEvent = function(eventObject) {
        ...
        if (eventObject.type == 'stateChange') {
            ...
            if (nico.player.state == 'playing') {
                ...
                if (nico.startedVideo) {
                    // nico.outAdAndplay();が実行されてないか確認？autoPlayPremiumがtrueではない
                    if (!nico.adPlay) {
                ...
                } else //一度目のload処理、autoplayなら再生する
                {
                    ...
                    if (nico.isShiSwfPlayer()) {
                        // 塗りつぶし背景削除
                        nico.video_base.video._visible = false;
                        changeAspect(nico.player.videoStream_width, nico.player.videoStream_height, false, true);
                    }
                    if (nico.adPlay) {
                        nico.player.updateVolume();
                    ...
                    movie_loaded = true;
                    nico.videowindow.loadingImage._visible = false;
                    nico.controller.controller_submenu.LargeScreenButton.enabled = true;
                    if (!nico.autoPlayPremium) {
                        if (!auto_play) {
                            ...
                            //auto_play true nico.autoPlay_so.data.flag false
                            outAdAndplay();
                        }
                    } else {
                        if (!nico.isInitBGM && bgms) {
                ...
                if (!thumbWatch) {
                    ...
                    endFunction = function() {
                        if (nico.ReplayFlag) {
                            ...
                            if (!nico.e && !nico.owner_thread_edit_mode && !nico.now_wayback_time) {
    ...
    nico.player.getStageWidth = function() {
        return Math.max(480, Stage.width);
    };
    nico.player.getStageHeight = function() {
        return Math.max(360, Stage.height);
    };
    nico.player.setLsize = function(isLargeScreen) {
        if (isLargeScreen) {
            this.setAspectVideoWindow();
            this._controller._y = this._mc._y + this._mc._height;
            this._controller.controll_bar._width = this.getStageWidth() - this.CONTROLLER_DEFAULT_X;
        } else {
            this._mc._xscale = 100;
            this._mc._yscale = 100;
            this._mc._x = this.VIDEOWINDOW_DEFAULT_X + video_normal_offset / 2;
            this._mc._y = this.VIDEOWINDOW_DEFAULT_Y;
            this._controller._y = this.CONTROLLER_DEFAULT_Y;
            this._controller.controll_bar._width = video_normal_width;
        }
        this._controller.controller_submenu._x = this._controller.controll_bar._width - (this._controller.controller_submenu._width + 10);
        this._controller.videoControlGard._width = this._controller.controller_submenu._x - 10;
        this._controller.hirobaControl.hirobaControlBG._width = this._controller.controller_submenu._x + this._controller.controller_submenu.NoMute_btn._x - 10;
        this._controller.hirobaControl.hirobaTitle._x = (this._controller.hirobaControl.hirobaControlBG._width - this._controller.hirobaControl.hirobaTitle._x) / 2;
        this.seeker_width = this._controller.controll_bar._width - (this._controller.controller_submenu._width + this.Seeker_mc._x + 20);
        this.Seeker_mc._width = this.seeker_width;
        this.SeekerHit_mc._width = this.Seeker_mc._width;
        this._controller.seekMessage._width = this.Seeker_mc._width;
        this.updateSeeker();
    };
    // シークタイム表示
    var pop_time_w = 42;
    ...
    var pop_time_r = 6;
    var pop_time_f = 8;
    nico.createEmptyMovieClip("pop_time", nico.getNextHighestDepth());
    nico.pop_time.createEmptyMovieClip("left", nico.pop_time.getNextHighestDepth());
    nico.pop_time.left.beginFill(0xf0f0f0, 100);
    nico.pop_time.left.moveTo(pop_time_f / 2, -pop_time_h / 2);
    nico.pop_time.left.lineTo(0, -pop_time_h / 2);
    nico.pop_time.left.curveTo(-pop_time_r, -pop_time_h / 2, -pop_time_r, -pop_time_h / 2 + pop_time_r);
    nico.pop_time.left.lineTo(-pop_time_r, pop_time_h / 2 - pop_time_r);
    nico.pop_time.left.curveTo(-pop_time_r, pop_time_h / 2, 0, pop_time_h / 2);
    nico.pop_time.left.lineTo(pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.left.endFill();
    nico.pop_time.left.lineStyle(1, 0x000000, 100, true, "none", "round", "miter", 4);
    nico.pop_time.left.moveTo(pop_time_f / 2, -pop_time_h / 2);
    nico.pop_time.left.lineTo(0, -pop_time_h / 2);
    nico.pop_time.left.curveTo(-pop_time_r, -pop_time_h / 2, -pop_time_r, -pop_time_h / 2 + pop_time_r);
    nico.pop_time.left.lineTo(-pop_time_r, pop_time_h / 2 - pop_time_r);
    nico.pop_time.left.curveTo(-pop_time_r, pop_time_h / 2, 0, pop_time_h / 2);
    nico.pop_time.left.lineTo(pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.createEmptyMovieClip("right", nico.pop_time.getNextHighestDepth());
    nico.pop_time.right.beginFill(0xf0f0f0, 100);
    nico.pop_time.right.moveTo(-pop_time_f / 2, -pop_time_h / 2);
    nico.pop_time.right.lineTo(0, -pop_time_h / 2);
    nico.pop_time.right.curveTo(pop_time_r, -pop_time_h / 2, pop_time_r, -pop_time_h / 2 + pop_time_r);
    nico.pop_time.right.lineTo(pop_time_r, pop_time_h / 2 - pop_time_r);
    nico.pop_time.right.curveTo(pop_time_r, pop_time_h / 2, 0, pop_time_h / 2);
    nico.pop_time.right.lineTo(-pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.right.endFill();
    nico.pop_time.right.lineStyle(1, 0x000000, 100, true, "none", "round", "miter", 4);
    nico.pop_time.right.moveTo(-pop_time_f / 2, -pop_time_h / 2);
    nico.pop_time.right.lineTo(0, -pop_time_h / 2);
    nico.pop_time.right.curveTo(pop_time_r, -pop_time_h / 2, pop_time_r, -pop_time_h / 2 + pop_time_r);
    nico.pop_time.right.lineTo(pop_time_r, pop_time_h / 2 - pop_time_r);
    nico.pop_time.right.curveTo(pop_time_r, pop_time_h / 2, 0, pop_time_h / 2);
    nico.pop_time.right.lineTo(-pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.createEmptyMovieClip("center", nico.pop_time.getNextHighestDepth());
    nico.pop_time.center.beginFill(0xf0f0f0, 100);
    nico.pop_time.center.moveTo(-pop_time_w / 2 + pop_time_r, pop_time_h / 2);
    nico.pop_time.center.lineTo(pop_time_w / 2 - pop_time_r, pop_time_h / 2);
    nico.pop_time.center.lineTo(pop_time_w / 2 - pop_time_r, -pop_time_h / 2);
    nico.pop_time.center.lineTo(-pop_time_w / 2 + pop_time_r, -pop_time_h / 2);
    nico.pop_time.center.endFill();
    nico.pop_time.center.lineStyle(1, 0x000000, 100, true, "none", "round", "miter", 4);
    nico.pop_time.center.moveTo(-pop_time_w / 2 + pop_time_r, pop_time_h / 2);
    nico.pop_time.center.lineTo(pop_time_w / 2 - pop_time_r, pop_time_h / 2);
    nico.pop_time.center.moveTo(pop_time_w / 2 - pop_time_r, -pop_time_h / 2);
    nico.pop_time.center.lineTo(-pop_time_w / 2 + pop_time_r, -pop_time_h / 2);
    nico.pop_time.createEmptyMovieClip("foot", nico.pop_time.getNextHighestDepth());
    nico.pop_time.foot.beginFill(0xf0f0f0, 100);
    nico.pop_time.foot.moveTo(-pop_time_f / 2 - 2, pop_time_h / 2);
    nico.pop_time.foot.lineTo(-pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.foot.lineTo(0, pop_time_h / 2 + pop_time_f);
    nico.pop_time.foot.lineTo(pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.foot.lineTo(pop_time_f / 2 + 2, pop_time_h / 2);
    nico.pop_time.foot.lineTo(pop_time_f / 2 + 2, pop_time_h / 2 - 1);
    nico.pop_time.foot.lineTo(-pop_time_f / 2 - 2, pop_time_h / 2 - 1);
    nico.pop_time.foot.endFill();
    nico.pop_time.foot.lineStyle(1, 0x000000, 100, true, "none", "round", "miter", 4);
    nico.pop_time.foot.moveTo(-pop_time_f / 2 - 2, pop_time_h / 2);
    nico.pop_time.foot.lineTo(-pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.foot.lineTo(0, pop_time_h / 2 + pop_time_f);
    nico.pop_time.foot.lineTo(pop_time_f / 2, pop_time_h / 2);
    nico.pop_time.foot.lineTo(pop_time_f / 2 + 2, pop_time_h / 2);
    nico.pop_time._visible = false;
    nico.pop_time.createTextField('time_text', nico.pop_time.getNextHighestDepth(), -1, -pop_time_h / 2, 2, 2);
    nico.pop_time.time_text.autoSize = "center";
    nico.pop_time.time_text.type = 'dynamic';
    nico.pop_time.time_text.setNewTextFormat(timeviewer_fmt);
    nico.player.Seeker_mc.onRollOut = function() {
        nico.pop_time._visible = false;
    };
    nico.player.Seeker_mc.onRollOver = function() {
        nico.pop_time._visible = true;
        nico.player.Seeker_mc.onMouseMove();
    };
    nico.player.Seeker_mc._onMouseMove = nico.player.Seeker_mc.onMouseMove;
    nico.player.Seeker_mc.onMouseMove = function() {
        nico.player.Seeker_mc._onMouseMove();
        if (nico.pop_time._visible) {
            var this_time = ((nico.player._controller._xmouse - nico.player.Seeker_mc._x) / nico.player.seeker_width) * nico.player._contentLength;
            this_time = Math.max(0, Math.min(nico.player._contentLength, this_time));
            var min = int(Math.floor(this_time / 60));
            var sec = int(this_time % 60);
            nico.pop_time.time_text.text = min + ':' + (sec < 10 ? '0' : '') + sec;
            nico.pop_time.center._width = Math.max(nico.pop_time.foot._width, nico.pop_time.time_text._width) - pop_time_r;
            nico.pop_time.left._x = -nico.pop_time.center._width / 2;
            nico.pop_time.right._x = nico.pop_time.center._width / 2;
            nico.pop_time._x = nico._xmouse;
            nico.pop_time._y = nico.controller._y;
            goNextDepth(nico.pop_time, nico);
        }
    };
};
...
function onPlayProc() {
    ...
    if (nico.player._contentLength != undefined) {
        var state = nico.player.__get__state();
        if ((state == "end" || state == "buffering") && nico.player.__get__playheadTime() >= nico.player._contentLength - 2) {
            nico.player.__set__playheadTime(0);
            // endのままになってしまうので更新
            setTimeout(function() {
...
function releaseFocus() {
    if (load_hiroba) {
        // 広場を読み込むとなぜかbgにフォーカスしてもすぐに外れる。(テキストエリアにフォーカスが有るとき)
...
function showInfoOnScreen(info, sec) {
    ...
    screen.wrapper_info.setTextFormat(red24b_fmt);
    screen.wrapper_info._x = nico.videowindow._x + nico.video_base._width * nico.video_base_video._xscale / 100 - screen.wrapper_info.textWidth;
    screen.wrapper_info._visible = true;
    setShowInfoOnScreenInterval(sec);
}
/* function setAutoPlayInfo(){ nico.videowindow._alpha = 100; screen.auto_play_info._visible = true; checkPlayerStart(); }  function clearAutoPlayInfo(){ nico.videowindow._alpha = 100; screen.auto_play_info._visible = false; } */
// 自動スムージング設定

function setAutoSmoothing(initialize) {
    if (auto_smoothing_off) {
        if (smoothing) {
            changeSmoothing(false, !initialize);
        }
    } else if (auto_smoothing) {
        var w = (video_base_w * nico.video_base_video._xscale / 100);
        var h = (video_base_h * nico.video_base_video._yscale / 100);
        if (w > nico.player.videoStream_width || h > nico.player.videoStream_height) {
            changeSmoothing(true, !smoothing && !initialize);
        } else {
            changeSmoothing(false, smoothing || initialize);
        }
    } else {
        if (initialize) {
...
function addLink(cand_links) {
    for (var i = 0; i < cand_links.length; i++) {
        ...
        if (mylists_len < 3 && cand_links[i].number.indexOf("mylist/") >= 0) {
            ...
            if (links[0].length == 0 && mylists_len == 0) {
                // 自動リンクは作成されてないはず
            ...
            if (!rep_flag) {
                if (links[0].length < max_auto_link) {
                    // max_auto_link以下なら
...
// 半角のみ
/* function searchLink(mode){ var headers = new Array("sm","am","fz","ca"); var cand_links = new Array(); for(var i=0; i<headers.length; i++){ var link = headers[i]; var ary; if(mode == "tags"){ary = video_tags;} else if(mode == "comments"){ary = nico.MessageSlots;} for(var j=0; j < ary.length; j++){ var original_mes, mes; if(mode == "tags"){original_mes = video_tags[j];} else if(mode == "comments"){original_mes = nico.MessageSlots[j]._text.text;} mes = original_mes; if(mes != ""){ while(mes.indexOf(headers[i]) >= 0){ var numbers = mes.substr(mes.indexOf(headers[i])+2, 8); for(var k=0; k < numbers.length; k++){ if(checkNum(numbers.charAt(k))){ link += numbers.charAt(k); } else{ break; } } if(link.length >= 3 && link != VIDEO){ var rep_flag = false;//すでに同じリンクがないか重複チェック for(var k=0; k<links[0].length; k++){ if(links[0][k].number == link){ rep_flag = true; break; } } if(!reg_flag){ for(var k=0; k<cand_links.length; k++){ if(cand_links[k].number == link){ rep_flag = true; break; } } } if(!rep_flag){ if(mode == "tags"){ original_mes = "[タグ] " + original_mes; cand_links.push({number: link, message: original_mes, status:"順番待ち"}); }else if(mode == "comments"){ for(var k=0; k<nico.Messages.length; k++){ if(nico.Messages[k]._slot == j){ cand_links.push({number: link, message: original_mes, user_id:fwMessages[k].user_id, status:"順番待ち"}); break; } } } } } link = headers[i]; mes = mes.substr(mes.indexOf(headers[i])+2); } } } }	 if(cand_links.length>0){return cand_links;} else{return undefined;} } */
...
function searchMessage(original_mes) {
    var headers = new Array("http://", "watch/", "mylist/", "sm", "am", "fz", "ca", "ax", "yo", "nm", "za", "zb", "zc", "zd", "cw", "fx", "na", "ig", "so");
    var cand_links = new Array();
...
/* //XMLのダウンロード function downloadXML(){  function connectBoard(update, wayback) { if(!ready_ownerthread && has_owner_thread) { connectBoard_ownerthread(); return undefined; } if(owner_thread_edit_mode) { return undefined; } if(MsgGetNow && last_resno > 0) { now_wayback_time = undefined; AddSystemMessage('コメント取得中のため、処理が中断されました。'); return undefined; } if(update && !Connection.isHttp()) { return undefined; } var v1 = -100; if(mm) { v1 = -1000; } else{ if(ContentLength >= 60) { v1 = -250; if(ContentLength >= 300) { v1 = -500; if(ContentLength >= 600) { v1 = -1000; } } } } resMax = Math.abs(v1); if(!NoMessages && Messages.length > 0 && !wayback) { v1 = Number(Messages[Messages.length - 1]._no) + 1; } ThreadTicket = undefined; closeInterval(); inputArea.ChatSendButton._visible = false; if(buttonMessageSlots) { buttonMessagesEnabledCheck(); } if(!update) { MsgGetNow = true; tabmenu.enabled = false; inputArea.enabled = false; } if(!wayback) { WaybackKey = undefined; wayback_time = undefined; } if(!sendCMsgGetThread_Time) { sendCMsgGetThread_Time = getTimer(); } Connection.SendCMsgGetThread(CurrentThreadID, NiwavidePacket.NIWAVIDE_PACKET_VERSION, v1, WaybackKey, wayback_time, UserID, nocompress); }  function connectBoard_ownerthread() { if(!sendCMsgGetThreadOwner_Time) { sendCMsgGetThreadOwner_Time = getTimer(); } Connection.SendCMsgGetThread(CurrentThreadID, NiwavidePacket.NIWAVIDE_PACKET_VERSION, OWNERTHREAD_MAX, undefined, undefined, UserID, nocompress, NiwavidePacket.FORK_OWNER); } v2.SendCMsgGetThread = function(threadid, version, resfrom, waybackkey, when, user, nocompress, fork, adminkey, force184) { var v3 = new XML(); var v2 = v3.createElement('thread'); if(threadid != undefined) { v2.attributes.thread = threadid; } if(version != undefined) { v2.attributes.version = version; } if(resfrom != undefined) { v2.attributes.res_from = resfrom; } if(waybackkey != undefined) { v2.attributes.waybackkey = waybackkey; } if(when != undefined) { v2.attributes.when = when; } if(user != undefined) { v2.attributes.user_id = user; } if(nocompress != undefined) { v2.attributes.no_compress = nocompress; } if(fork != undefined) { v2.attributes.fork = fork; } if(adminkey != undefined) { v2.attributes.adminkey = adminkey; } if(force184 != undefined) { v2.attributes.force_184 = force184; } v3.appendChild(v2); this.SendXml(v3); }; v2.SendXml = function(xml) { if(this.bufferingLevel != 0) { this.XMLBuffer.appendChild(xml.firstChild); } else{ this.SendXmlLower(xml); } };  v2.SendXmlLower = function(xml) { var v2 = xml.toString(); if(!this.httpConnection && !this.connected) { this.SendBuffer += v2; if(!this.connecting) { this.DoConnect(); } } else{ if(this.httpConnection) { this.connecting = true; xml.sendAndLoad(this.serverHost, this.httpReceiver); } else{ this.xmlsocket.send(v2); } if(this.onSend != undefined) { this.onSend(xml); } } }; v2.DoConnect = function() { if(this.httpConnection) { return undefined; } if(!this.connecting && !this.connected) { this.connecting = true; if(this.onConnecting != undefined) { this.onConnecting(this.serverHost, this.serverPort); } this.xmlsocket.connect(this.serverHost, this.serverPort); } }; var fileRef = new flash.net.FileReference(); var fileListener = new Object(); fileListener.onSelect = function(file) { // 参照・保存ダイアログで開く・保存を選択したときの処理 showInfoOnMainBar('onSelect: ' + file.name); }; fileListener.onCancel = function(file) { // 参照・保存ダイアログでキャンセルを選択したときの処理 showInfoOnMainBar('onCancel: ' + file.name); }; fileListener.onOpen = function(file) { // ダウンロード開始時の処理 showInfoOnMainBar('onOpen: ' + file.name); }; fileListener.onProgress = function(file, nLoadedBytes, nTotalBytes) { // ダウンロード中の処理 showInfoOnMainBar('onProgress' + file.name); }; fileListener.onComplete = function(file) { // ダウンロード完了時の処理 showInfoOnMainBar('onComplete: ' + file.name); }; fileListener.HTTPError = function(file) { // 入出力エラーでダウンロード失敗時の処理 showInfoOnMainBar('onHTTPError: ' + file.name); }; fileListener.onIOError = function(file) { // 入出力エラーでダウンロード失敗時の処理 showInfoOnMainBar('onIOError: ' + file.name); }; fileListener.onSecurityError = function(file, error) { // セキュリティエラーでダウンロード失敗時の処理 showInfoOnMainBar('onSecurityError: '+ error); }; fileRef.addListener(fileListener); fileRef.download(_url, VIDEO + "." + movie_type); } */
...
function deleteExpID(ary, sort_status) {
    ...
    if (max_ng_id > 0) {
        if (result_ary.length > max_ng_id) {
            // NGIDが規定数を超えていたら
    ...
    if (ng_id_expires > 0) {
        ...
        for (var i = result_ary.length - 1; i >= 0; i--) {
            if (now - result_ary[i].date >= ng_id_expires * (24 * 60 * 60 * 1000)) {
                // 期限切れを削除
                result_ary.pop();
            } else {
                break;
...
function deleteExpKakikomi(ary, sort_status) {
    ...
    var result_ary = ary.slice();
    if (max_kaki_komi > 0) {
        if (result_ary.length > max_kaki_komi) {
            // kakikomiが規定数を超えていたら
    ...
    if (kaki_komi_expires > 0) {
        ...
        var now = myDate.getTime();
        for (var i = result_ary.length - 1; i >= 0; i--) {
            if (now - result_ary[i].date >= kaki_komi_expires * (24 * 60 * 60 * 1000)) {
                // 期限切れを削除
                result_ary.pop();
            } else {
                break;
...
// 目的のムービークリップのすぐ上に位置する深度を空けて取得

function getNextDepth(target) {
    var parent = target._parent;
    var depth = target.getDepth();
    if (1048575 - 1000 < depth) {
        return depth;
    }
    if (0 < depth && parent.getInstanceAtDepth(depth - 1) == undefined) {
        target.swapDepths(depth - 1);
        return depth;
    }
    var mc = parent.getInstanceAtDepth(depth + 1);
    if (mc != undefined) {
        getNextDepth(mc);
    }
    return depth + 1;
...
function showAttentionDialog(msg, func_clk, func_del) {
    attentionDialog.onEnterFrame = function() {
        click_valid = false;
        if (key_down || (mouse_down && !attentionDialog.hitTest(_root._xmouse, _root._ymouse, true))) {
            attentionDialog._visible = false;
            attentionDialog.removeMovieClip();
            attentionDialog.onEnterFrame = undefined;
            func_del();
        }
    };
    attentionDialog.onRelease = function() {
        attentionDialog._visible = false;
        func_clk();
    };
    attentionDialog.text.text = '\n\n' + msg + "\n(クリックで継続)";
    attentionDialog._x = (Stage.width - attentionDialog._width) / 2;
    attentionDialog._y = (Stage.height - attentionDialog._height) / 2;
    attentionDialog._visible = true;
    goTopDepth(attentionDialog);
}
// 目的のムービークリップのすぐ下に位置する深度を空けて取得

function getPrevDepth(target) {
    target.swapDepths(getNextDepth(target));
    return target.getDepth() - 1;
}
// 1番目のムービークリップを2番のすぐ上に位置するよう移動

function goNextDepth(mcPrev, mcNext) {
    if (mcPrev.getDepth() != mcNext.getDepth() + 1) {
        mcPrev.swapDepths(mcPrev._parent.getNextHighestDepth());
        mcPrev.swapDepths(getNextDepth(mcNext));
    }
}
// 1番目のムービークリップを2番のすぐ下に位置するよう移動

function goPrevDepth(mcPrev, mcNext) {
    if (mcPrev.getDepth() != mcNext.getDepth() - 1) {
        mcPrev.swapDepths(mcPrev._parent.getNextHighestDepth());
        mcPrev.swapDepths(getPrevDepth(mcNext));
    }
}
// ムービークリップの並び順を正す

function sortDepth1(mcPrev, mcNext) {
    if (mcPrev.getDepth() > mcNext.getDepth()) {
        mcNext.swapDepths(getNextDepth(mcPrev));
    }
}
// ムービークリップの並び順を正す

function sortDepth2(mcPrev, mcNext) {
    if (mcPrev.getDepth() > mcNext.getDepth()) {
        mcPrev.swapDepths(getPrevDepth(mcNext));
    }
}
...
function goBottomDepth(target) {
    var parent = target._parent;
    var min_mc = target;
    var min = 1048575 - 1000;
    for (var idx in parent) {
        var mc = parent[idx];
        if (mc instanceof MovieClip) {
            if (mc.getDepth() < min && mc.getDepth() >= 0) {
                min_mc = mc;
                min = mc.getDepth();
            }
        }
    }
    target.swapDepths(getPrevDepth(min_mc));
}
// 目的のムービークリップを最前面にする

function goTopDepth(target) {
    var parent = target._parent;
    var max_mc = target;
    var max = 0;
    if (1048575 - 100 < target.getDepth()) {
        return;
    }
    for (var idx in parent) {
        var mc = parent[idx];
        if (mc instanceof MovieClip && mc != target) {
            if (mc.getDepth() >= max && mc.getDepth() <= 1048575 - 1000) {
                max_mc = mc;
                max = mc.getDepth();
            }
        }
    }
    target.swapDepths(getNextDepth(max_mc));
}
// おもにデバッグ用
...
timeLine.getflv.onEnterFrame = function() {
    if (getflv_status == "ready") {
        ...
        // flvplayer.swfの読み込み終了
        if (total > 0 && total == loaded && nico.isStartupPlayer) {
            // いくつかのfunctionを上書きしてしまう
            replaceFunction();
            // レイアウト変更等が終わったので表示
            // プレミアムの自動再生をいじれるように。
            nico.System_menu.autoPlayCheckBox.enabled = true;
            ...
            if (movie_type == 'mp4' && !detectFlashVer(9, 0, 115) && !eco) {
                var mes = 'この動画を視聴するためにはFlashPlayerをバージョンアップする必要がありま
						  す。\n<CENTER><U><FONT color=\'#0000FF\'><A href=\'http://www.adobe.com/shockwave/download/index.cgi?Lang=Japanese&
						  amp;P1_Prod_Version=ShockwaveFlash\'>最新のFlashPlayerをダウンロードする</A&
						  gt;</FONT></U></CENTER>\n\n';
                if (iee) {
...
/* //自動リンク監視 timeLine.createEmptyMovieClip("auto_link",70); function setAutoLinkInterval(){ timeLine.auto_link.onEnterFrame = function(){ if(check_interval % 60 == 0){ if(auto_link && links.length <= max_auto_link-1){ var cand_links = searchLink("comments"); if(cand_links != undefined){ addLink(cand_links); } }else{ delete this.onEnterFrame; } } }; } */
// 自動コメント収集監視
...
timeLine.change_title.onEnterFrame = function() {
    ...
    if (check_interval % 60 == 0) {
        ...
        if ((total <= now && (total > 0 || total_bytes_backup > 0)) || local_file_found) {
            // 読み込み中と思われる
            delete this.onEnterFrame;
            if (local_file_found) {
...
flv_index.onData = function(src) {
    ...
    // 動画が見つからない
    if (local_server_num < local_server_index.length - 1) {
        // 次のサーバーをチェック
        local_server_num++;
        check_flv_status = "ready";
        return;
    } else {
        // 最後のサーバーまでチェックしたので普通にニコニコに接続
...
/* //デバッグ用 main_bar.createTextField("debug_info",1000,5,40,290,19); main_bar.debug_info.type = "dynamic"; main_bar.debug_info.border = true; main_bar.debug_info.selectable = false; main_bar.debug_info.background = true; main_bar.debug_info.autoSize = true; main_bar.debug_info.setTextFormat(red10_fmt); function showDebugInfo(info){ main_bar.debug_info.text = info; }*/
...
/* //自動再生禁止時にプレイヤーがスタートするのを監視 timeLine.createEmptyMovieClip("check_player_start",170); function checkPlayerStart(){ timeLine.check_player_start.onEnterFrame = function(){ var state = nico.player.__get__state(); if(state == "playing" || state == "connectionError" || nico.player.dummy_active){ clearAutoPlayInfo(); delete this.onEnterFrame; } }; } */
// 強引にシークする
